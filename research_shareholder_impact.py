...
def run_no_filter_research():
    ...
    configs = [
        {"name": "Official (50% Group / 50% Ind)", "group_pct": 0.5, "ind_min": 0.5},
        {"name": "No Shareholder Filters (Pure RSNP)", "group_pct": 1.0, "ind_min": 0.0},
        {"name": "Strict (100% Industry Decrease Only)", "group_pct": 1.0, "ind_min": 1.0}
    ]
    ...
    for config in configs:
        ...
        strategy = ContrarianBreadthStrategy(
            data_handler=dh,
            num_stocks=15,
            industry_group_top_pct=config['group_pct'],
            industry_decrease_min_pct=config['ind_min'],
            rsnp_threshold=0.4,
            rsi_threshold=40,
    ...
    # 4. Report
    print("\n" + "="*120)
    print("IMPACT OF SHAREHOLDER FILTERS (RSNP vs CONTRARIAN RSNP)")
    print("="*120)
    
    metrics = ["Absolute Return", "CAGR", "Max Drawdown", "Sharpe Ratio"]
    headers = [c['name'] for c in configs]
    
    print(f"{'Metric':<20} | {headers[0]:>30} | {headers[1]:>30} | {headers[2]:>30}")
    print("-" * 128)
    ...
    for m in metrics:
        ...
        print(f"{m:<20} | {v0:>30} | {v1:>30} | {v2:>30}")
    print("="*120)

if __name__ == "__main__":
    run_no_filter_research()
