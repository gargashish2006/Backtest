...
def run_industry_basket_v3_simulation(dh, scale_factor=10000.0):
    ...
    for date in sim_dates:
        ...
        if date in rebalance_dates:
            ...
            # 3. Top 35% Industries within Top Groups
            ...
            top_industries = industry_stats.head(int(len(industry_stats) * 0.35)).index.tolist()
            ...
            selected_weights = {}
            num_industries = len(top_industries)
            weight_per_industry = 1.0 / num_industries
...
def run_industry_basket_v3_study():
    repo_root = Path("/Users/shubhrakasana/Desktop/oleander/olgo_mobile/repos/Backtest/precision-backtest-engine")
    dh = DataHandler(repo_root / "database/price_data_cleaned.parquet")
    dh.load_data()
    
    print("Running Relative Industry Basket Simulation (35% Group / 35% Industry)...")
    nav_basket = run_industry_basket_v3_simulation(dh)
    
    print(f"Final NAV (Relative Industry Basket): {nav_basket.iloc[-1]:.2f}")
    ...
    nav_basket.plot()
    plt.title("Relative Industry Basket: Top 35% Groups -> Top 35% Ind -> 3 Stock (High MC)")
    plt.ylabel("NAV (Base 100)")
    plt.grid(True)
    plt.savefig(repo_root / "relative_industry_basket_nav.png")
    print(f"\nPlot saved to: relative_industry_basket_nav.png")

if __name__ == "__main__":
    run_industry_basket_v3_study()
