...
def create_content_slide(prs, title, subtitle, bullets, image_path=None, page=0, disclaimer=None):
    ...
    if image_path:
        # Image Left, Text Right? Or vice versa. Let's stick to consistent text left for now, but for icons slide, image left looks cool.
        # Actually in previous versions, I put text left, image right. Let's flip for this one or keep consistent.
        # Let's keep consistent: Text Left, Image Right.
        text_width = Inches(6)
        img_left = Inches(7)
        img_top = Inches(2)
...
create_content_slide(
    ...
    [
        ...
        "Rebalancing: Quarterly (With Dynamic Exits)",
...
# Save
output_path = '/Users/shubhrakasana/Desktop/oleander/olgo_mobile/repos/Backtest/precision-backtest-engine/Champion_Strategy_Presentation_v6.pptx'
prs.save(output_path)
print(f"Presentation v6 saved to: {output_path}")
