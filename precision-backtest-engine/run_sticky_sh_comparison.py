...
def run_sticky_sh_comparison():
    ...
    variations = [
        ("Baseline (Original Champion)", False),
        ("Sticky SH (1-Quarter)", True)
    ]
    
    for label, sticky_sh in variations:
        print(f"\n--- Running Variation: {label} ---")
        port = Portfolio(10000000)
        # Using 0-year history filter (Original Champion)
        strategy = ContrarianBreadthStrategy(dh, min_history_years=0.0, sticky_sh_filter=sticky_sh)
    ...
    print("\n" + "="*60)
    print(f"STICKY SHAREHOLDER VARIATION RESULTS (2017 - 2026)")
    print("="*60)
