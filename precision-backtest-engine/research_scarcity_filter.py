...
class ScarcityCashStrategy(ContrarianBreadthStrategy):
    """Overrides calculate_selection to implement Cash Call based on Industry Scarcity."""
    def __init__(self, *args, scarcity_threshold=0.15, **kwargs):
    ...
    def calculate_selection(self, date: pd.Timestamp) -> pd.Series:
        ...
        # Total Industries considered in the universe (approx)
        ...
        if qualify_pct < self.scarcity_threshold:
            print(f"CASH CALL on {date.date()}: Only {qualify_pct:.1%} industries qualified (Threshold {self.scarcity_threshold:.0%})")
            self.cash_calls.append({'date': date, 'pct': qualify_pct})
            return {} # Empty dict = 100% Cash
...
def run_scarcity_research():
    ...
    # 2. Scarcity Filter Strategy
    print("\nRunning Scarcity Filter Strategy (Cash if < 15% Ind Qualify)...")
    strat1 = ScarcityCashStrategy(dh, num_stocks=15, scarcity_threshold=0.15)
    ...
    # 3. Scarcity Filter Strategy (10%)
    print("\nRunning Scarcity Filter Strategy (Cash if < 10% Ind Qualify)...")
    strat2 = ScarcityCashStrategy(dh, num_stocks=15, scarcity_threshold=0.10)
    ...
    print("\n" + "="*100)
    print("IMPACT OF INDUSTRY SCARCITY CASH FILTER (<15% vs <10%)")
    print("="*100)
    metrics = ["Absolute Return", "CAGR", "Max Drawdown", "Sharpe Ratio"]
    print(f"{'Metric':<20} | {'Official':>20} | {'<15% Filter':>20} | {'<10% Filter':>20}")
    print("-" * 100)
    for m in metrics:
        print(f"{m:<20} | {res0[m]:>20} | {res1[m]:>20} | {res2[m]:>20}")
    print("="*100)

if __name__ == "__main__":
    run_scarcity_research()
