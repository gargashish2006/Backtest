"""
MCPS Synchronized Lookback Sensitivity Analysis:
Tests MCPS (12 stocks) with synchronized lookbacks (2Q, 3Q, 4Q, 5Q) 
for BOTH the signal and the industry group breadth filter.
"""
import pandas as pd
...
def run_lookback(label, lookback_q):
    print(f"Running {label} (Synchronized {lookback_q}Q)...")
    p = Portfolio(10_000_000)
    # Using MCPSStrategy with synchronized lookbacks
    s = MCPSStrategy(dh, group_top_pct=0.50, num_stocks=12, max_per_industry=3, 
                     mcps_lookback_quarters=lookback_q,
                     group_lookback_quarters=lookback_q)
...
# ── Print Table ───────────────────────────────────────────────────────────────
print("\n" + "=" * 110)
header = f"{'Metric':<20} | {'CS15':^12} | " + " | ".join(f"{l:^15}" for l in nav_history.keys())
print(header)
print("-" * 110)
for k in cs15_stats.keys():
    row = f"{k:<20} | {cs15_stats[k]:^12} | "
    row += " | ".join(f"{results[label][k]:^15}" for label in nav_history.keys())
    print(row)
print("=" * 110)
...
ax.set_title('MCPS Synchronized Lookback Sensitivity Analysis (2Q - 5Q)', fontsize=16, color='white', pad=20)
...
out_path = repo_root / "mcps_synced_lookback_analysis.png"
plt.tight_layout()
