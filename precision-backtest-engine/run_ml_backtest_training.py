...
def run_ml_backtest_training():
    ...
    # 2. Strategy
    model_path = repo_root / "models/ml_model_rf.pkl"
    if not model_path.exists():
        print("ML Model not found!")
        return
        
    print(f"Initializing ML Strategy with model: {model_path.name}")
    strategy = MLStrategy(dh, model_path)
    
    # 3. Backtest Period (IN SAMPLE TRAINING)
    ...
    print(f"Backtesting TRAINING PERIOD from {start_date} to {end_date}...")
    ...
    eng.run(start_date, end_date, strategy.calculate_selection, rebalance_dates, verbose=True)
    
    # 5. Results
    nav_df = pd.DataFrame(port.nav_history)
    output_path = repo_root / "outputs/ml_strategy_training_nav.csv"
    nav_df.to_csv(output_path, index=False)
    
    stats = calculate_metrics(nav_df)
    
    print("\n" + "="*60)
    print("ML STRATEGY PERFORMANCE (TRAINING PERIOD: 2017-2022)")
    print("="*60)
