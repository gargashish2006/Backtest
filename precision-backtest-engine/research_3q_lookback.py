...
def run_3q_variation():
    ...
    print("Running Champion Strategy with 3-Quarter (9-Month) Shareholder Lookback...")
    warnings.filterwarnings('ignore')

    # Champion Params with 3-Quarter Lookback
    strat = ContrarianBreadthStrategy(dh,
                                    ...
                                    shareholder_lookback_quarters=3) # VARIATION: 3 Quarters (9 Months)
    ...
    output_path = repo_root / "outputs/lookback_3q_nav.csv"
    nav_df.to_csv(output_path, index=False)
    
    stats = calculate_metrics(nav_df)
    
    print("\n" + "="*60)
    print("3-QUARTER LOOKBACK VARIATION PERFORMANCE")
    print("="*60)
    ...
    print("="*60)
    print(f"{'Metric':<20} | {'3-Quarter (9M)':>15} | {'Champion (1Y)':>15}")
    print("-" * 55)
    ...
    if baseline_path.exists():
        ...
        plt.plot(var_nav, label='Variation (9M/3Q)', linewidth=2)
        plt.title('Strategy Comparison: Shareholder Lookback (9 Months vs 1 Year)')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.savefig(repo_root / "outputs/lookback_3q_comparison.png")
        print(f"Chart saved to: {repo_root / 'outputs/lookback_3q_comparison.png'}")
