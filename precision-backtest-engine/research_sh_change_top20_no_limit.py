...
def run_sh_change_top20_unlimited_sim(dh, scale_factor=10000.0):
    ...
    for date in sim_dates:
        ...
        if date in rebalance_dates:
            ...
            if sh_trend.empty: 
                ...
                if not stocks_info:
                    ...
                    if final_pool.empty:
                        ...
                        # Select Top 20 stocks by rank (most negative first) - NO INDUSTRY LIMIT
                        selection = final_pool.sort_values('sh_change_pct', ascending=True).head(20)['isin'].tolist()
            ...
            if selection:
                ...
                weight_per_stock = 1.0 / len(selection)
                
                for isin in selection:
                    p = prices.get(isin)
                    if p:
                        qty = int((invest_val * weight_per_stock) / (p * 1.0065))
                        if qty > 0:
...
def run_study():
    ...
    print("Running Concentrated SH Change % Ranking Simulation (Top 20, No Industry Limit)...")
    nav_model = run_sh_change_top20_unlimited_sim(dh)
    ...
    print(f"Final NAV (Concentrated Top 20 Model): {nav_model.iloc[-1]:.2f}")
    print(f"Final NAV (Top 1000 Bench): {nav_bench.iloc[-1]:.2f}")
    
    plt.figure(figsize=(10, 6))
    nav_model.plot(label='SH Change % Rank (Top 20 / Pure Rank)')
    nav_bench.plot(label='Top 1000 Benchmark', alpha=0.7)
    plt.title("Concentrated SH Change % Ranking (Top 20) vs Top 1000 Bench")
    plt.ylabel("Value (Base 100)")
    plt.legend()
    plt.grid(True)
    plt.savefig(repo_root / "outputs/sh_change_top20_no_limit_nav.png")
    print("\nPlot saved to: outputs/sh_change_top20_no_limit_nav.png")
