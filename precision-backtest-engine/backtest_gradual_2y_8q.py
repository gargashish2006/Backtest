...
def run_gradual_2y_8q():
    ...
    q_allot = 12.5 * scale_factor # 100 / 8 = 12.5
    
    fee_model = FeeModel(transaction_fee_rate=0.0015, impact_cost_rate=0.005)
    tax_man = TaxManager(stcg_rate=0.20, ltcg_rate=0.125)
    portfolio = Portfolio(initial_cash=initial_cash)
    
    # Strategy Config: Top 10 Industries, 3 stocks each (Total 30)
    strategy = StructuralAlphaStrategy(dh, num_stocks=30, max_per_industry=3)
    
    # 2. Dates
    rebalance_dates = []
    # Need to go further in time to see the 8Q entry + some holding
    ...
    end_date = all_dates[-1] # Run until max data
    ...
    print(f"Starting Ultra-Long Gradual Entry (30 stocks, 12.5% x 8) from {start_date.date()}...")
    ...
    for date in sim_dates:
        ...
        # Daily Benchmark NAV
        bench_val = 0.0
        b_prices = dh.top_100_bench[dh.top_100_bench['date'] <= date] # Using top 1000 bench if available, or dh.top_1000_bench
        # Let's check dh for correct benchmark attribute
        ...
        if date in rebalance_dates:
            ...
            if current_q_idx <= 8:
                ...
                # 2-Year Rolling (Hold 8 Quarters)
                ...
                if old_map:
                    ...
                    # Reinvest the specific cash from this liquidation
    ...
    plt.plot(df_nav['nav'] / scale_factor, label="Ultra-Long Strategy (30 stock, 2Y Hold)")
    plt.plot(df_bench['nav'] / scale_factor, label="Staggered Benchmark (8Q Entry then Static)", alpha=0.7, color='gray')
    plt.title(f"Ultra-Long Gradual Entry (2Y Hold / 8Q Stagger)\nStrategy: {final_nav:.1f} | Bench: {final_bench:.1f}")
    plt.ylabel("Portfolio Value (Base 100)")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(repo_root / "gradual_2y_8q_results.png")
    
    print(f"\nULTRA-LONG GRADUAL ENTRY COMPLETE")
