...
def run_cs15_backup():
    ...
    # Overwrite the clean database with the original BEFORE TT scrape
    backup_path = repo_root / "database/shareholding_patterns_backup_pre_tt.parquet"
    if not backup_path.exists():
        print(f"Backup path not found: {backup_path}")
        return
        
    dh.shareholding_df = pd.read_parquet(backup_path)
    ...
    print("Running CS15 Backtest on BACKUP pre-Tickertape Data...")
    ...
    print("\n" + "="*40)
    print("CS15 STRATEGY (BACKUP DATA)")
    print("="*40)
