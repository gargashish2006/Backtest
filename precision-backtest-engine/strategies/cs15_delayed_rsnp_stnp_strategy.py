...
class CS15DelayedRsnpStnpStrategy(CS15Strategy):
    """
    CS15_delayed_rsnp_stnp: combines the delayed RSNP window with STNP.

    RSNP (delayed, same as CS15_delayed):
        % of stocks beating NIFTY 500 over (curr_q_end - 1yr) -> curr_q_end

    STNP (short-term, non-overlapping):
        % of stocks beating NIFTY 500 over curr_q_end -> signal_date

    Score = RSNP - STNP, threshold > 0, ranked descending.

    The two windows are non-overlapping:
        [curr_q_end - 1yr  ...  curr_q_end] [curr_q_end  ...  signal_date]
              RSNP (long)                          STNP (short)
    """
    ...
    def calculate_selection(self, date: pd.Timestamp) -> Dict[str, float]:
        ...
        if not rsnp_end_candidates:
            return {}
        rsnp_end = max(rsnp_end_candidates)                          # curr_q last trading day
        ...
        if not rsnp_start_candidates:
            return {}
        rsnp_start = max(rsnp_start_candidates)                      # rsnp_end - 1yr

        # STNP window: rsnp_end -> signal_date
        stnp_start = rsnp_end
        stnp_end = actual_signal_date

        if stnp_start >= stnp_end:
            return {}
        ...
        bench_rsnp = bench_return_between(rsnp_start, rsnp_end)
        bench_stnp = bench_return_between(stnp_start, stnp_end)
        ...
        p_rsnp_start = get_map(rsnp_start)
        p_rsnp_end   = get_map(rsnp_end)    # also stnp_start
        p_stnp_end   = get_map(stnp_end)    # signal_date

        # 3. RSNP and STNP per industry
        industry_scores = []
        for ind in qualified_industries:
            ...
            for i in isins:
                ...
                c1s, c0s = p_stnp_end.get(i), p_rsnp_end.get(i)
        ...
        # 4. Universe & liquidity at signal date (unchanged)
