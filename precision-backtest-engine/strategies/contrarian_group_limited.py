...
class ContrarianGroupLimitedStrategy:
    """
    Variant of the Champion Contrarian Breadth Strategy:
    Adds an additional constraint: Max 3 stocks per INDUSTRY GROUP.
    """
    ...
    def calculate_selection(self, date: pd.Timestamp) -> Dict[str, float]:
        """Returns target weights with added Group Limit constraint."""
        ...
        # 5. Stock Selection with Industry Group Limit
        selected_isins = []
        ...
        for ind in ind_ranked['industry']:
            ...
            group_name = next((g for i, g in self.dh.isin_to_group.items() if self.dh.isin_to_industry.get(i) == ind), None)
            if not group_name: continue
            
            # Check Group Limit
            current_group_count = group_counts.get(group_name, 0)
            if current_group_count >= self.max_per_group: continue
            ...
            # Calculate how many more we can pick for this industry vs group limit
            quota_ind = self.max_per_industry
            quota_grp = self.max_per_group - current_group_count
            final_quota = min(quota_ind, quota_grp)
            
            top_for_ind = ind_universe.head(final_quota)['isin'].tolist()
