...
def create_full_chart_slide(prs, title, subtitle, image_path, bullets=[], page=0, text_on_bottom=False):
    ...
    if bullets:
        if text_on_bottom:
             b_box = slide.shapes.add_textbox(Inches(0.5), Inches(6), Inches(12), Inches(1))
        else:
             # Overlay on top right if suitable, or create a sidebar
             # For quarterly chart (wide), maybe overlay is bad.
             pass
...
# Update text to match Rf=6% Sharpe ~0.83
create_content_slide(
    prs,
...
# Slide 8: Quarterly Returns (New Slide)
create_full_chart_slide(
    prs,
...
# Slide 9: Dynamic Edge (Conclusion)
...
# Save
output_path = '/Users/shubhrakasana/Desktop/oleander/olgo_mobile/repos/Backtest/precision-backtest-engine/Champion_Strategy_Presentation_v4.pptx'
prs.save(output_path)
print(f"Presentation v4 saved to: {output_path}")
