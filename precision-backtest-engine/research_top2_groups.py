...
class Top2GroupsStrategy(ContrarianBreadthStrategy):
    """
    Variation:
    Select Top 2 Industry Groups (by Shareholder Decrease) instead of Top 50%.
    """
    def calculate_selection(self, date: pd.Timestamp) -> Dict[str, float]:
        ...
        # VARIATION: PICK TOP 2 GROUPS FIX
        num_to_pick = 2
...
def run_top2_groups_variation():
    ...
    print("Running Champion Strategy with TOP 2 INDUSTRY GROUPS Variation...")
    warnings.filterwarnings('ignore')

    strat = Top2GroupsStrategy(dh,
                               num_stocks=15, 
                               max_per_industry=3,
                               universe_size=1000, 
                               liquidity_threshold_pct=0.00005,
                               industry_group_top_pct=0.50, # Ignored, overridden in logic
    ...
    output_path = repo_root / "outputs/top2_groups_nav.csv"
    nav_df.to_csv(output_path, index=False)
    
    stats = calculate_metrics(nav_df)
    
    print("\n" + "="*60)
    print("TOP 2 INDUSTRY GROUPS VARIATION PERFORMANCE")
    print("="*60)
    ...
    print("="*60)
    print(f"{'Metric':<20} | {'Top 2 Groups':>15} | {'Champion':>15}")
    print("-" * 55)
    ...
    if baseline_path.exists():
        ...
        plt.plot(champ_nav, label='Champion (Top 50% Groups)', alpha=0.7)
        plt.plot(var_nav, label='Variation (Top 2 Groups)', linewidth=2)
        plt.title('Strategy Comparison: Top 2 Groups vs Top 50% Groups')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.savefig(repo_root / "outputs/top2_groups_comparison.png")
        print(f"Chart saved to: {repo_root / 'outputs/top2_groups_comparison.png'}")

if __name__ == "__main__":
    run_top2_groups_variation()
