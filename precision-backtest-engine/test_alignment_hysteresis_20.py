"""
Alignment Benchmark Switching with Hysteresis (20% Buffer)
Rule:
- Switch to T1000 if Breadth(Mid) > Breadth(T200) + 20%
- Switch to T200 if Breadth(T200) > Breadth(Mid) + 20%
...
def get_segment_breadth(date):
    ...
    valid_dates = [d for d in all_dates if d <= signal_date]
    if not valid_dates: return 0, 0
    actual_signal_date = max(valid_dates)
...
p_hyst_20 = initial_cash
p_hyst_10 = initial_cash
p_align = initial_cash

history = []

curr_holding_hyst_20 = []
curr_regime_hyst_20 = "Top 1000" # Initial default

curr_holding_hyst_10 = []
curr_regime_hyst_10 = "Top 1000"
...
for i, rd in enumerate(rdates):
    if i > 0:
        ...
        p_hyst_20 = calc_nav(curr_holding_hyst_20, p_hyst_20)
        p_hyst_10 = calc_nav(curr_holding_hyst_10, p_hyst_10)
    ...
    # 1. Standard Alignment (Threshold 0)
    if b_a > b_b:
        curr_holding_align = isins_200
    else:
        curr_holding_align = isins_1000
        
    # 2. Hysteresis Alignment (10% Buffer)
    if curr_regime_hyst_10 == "Top 1000":
        if b_a > (b_b + 0.10): curr_regime_hyst_10 = "Top 200"
    elif curr_regime_hyst_10 == "Top 200":
        if b_b > (b_a + 0.10): curr_regime_hyst_10 = "Top 1000"
    curr_holding_hyst_10 = isins_200 if curr_regime_hyst_10 == "Top 200" else isins_1000

    # 3. Hysteresis Alignment (20% Buffer)
    if curr_regime_hyst_20 == "Top 1000":
        if b_a > (b_b + 0.20): curr_regime_hyst_20 = "Top 200"
    elif curr_regime_hyst_20 == "Top 200":
        if b_b > (b_a + 0.20): curr_regime_hyst_20 = "Top 1000"
    curr_holding_hyst_20 = isins_200 if curr_regime_hyst_20 == "Top 200" else isins_1000
        
    history.append({
        'date': rd,
        'NAV_Hyst_20': p_hyst_20,
        'NAV_Hyst_10': p_hyst_10,
        'NAV_Align': p_align,
        'Regime_Hyst_20': curr_regime_hyst_20,
        'B_Diff': b_a - b_b
    })
...
print(f"{'Hysteresis (10% Buf)':<20} | {get_stats(df_hist['NAV_Hyst_10'])[0]:^15} | {get_stats(df_hist['NAV_Hyst_10'])[1]:^15}")
print(f"{'Hysteresis (20% Buf)':<20} | {get_stats(df_hist['NAV_Hyst_20'])[0]:^15} | {get_stats(df_hist['NAV_Hyst_20'])[1]:^15}")
print("=" * 60)
...
ax.plot(df_hist.index, df_hist['NAV_Hyst_10'] / initial_cash * 100, color='#e71d36', alpha=0.5, label='10% Buffer')
ax.plot(df_hist.index, df_hist['NAV_Hyst_20'] / initial_cash * 100, color='#00d4ff', linewidth=3, label='20% Buffer')

# Plot Buffer Regions for 20%
for i in range(len(df_hist)-1):
    d1 = df_hist.index[i]
    d2 = df_hist.index[i+1]
    reg = df_hist.iloc[i]['Regime_Hyst_20']
    color = '#00d4ff' if reg == 'Top 200' else '#6bcb77'
    ax.axvspan(d1, d2, color=color, alpha=0.1)

ax.set_title("Alignment Switching with Hysteresis (20% Buffer)", color='white', fontsize=16)
ax.tick_params(colors='white')
ax.grid(True, color='#222222')
ax.legend(facecolor='#1a1a2e', labelcolor='white')

out_path = repo_root / "alignment_hysteresis_20_switcher.png"
plt.tight_layout()
