...
Compares MCPS (15 stocks) vs MCPS (12 stocks).
...
# ── Run Test (12 Stocks) ──────────────────────────────────────────────────────
stats_12, nav_12 = run_variant("MCPS 12 Stocks", 12)
...
# ── Print Table ───────────────────────────────────────────────────────────────
print("\n" + "=" * 80)
print(f"{'Metric':<20} | {'CS15 (15)':^12} | {'MCPS (15)':^12} | {'MCPS (12)':^12}")
print("-" * 80)
for k in cs15_stats.keys():
    print(f"{k:<20} | {cs15_stats[k]:^12} | {stats_15[k]:^12} | {stats_12[k]:^12}")
print("=" * 80)
...
ax.plot(nav_12.index, nav_12.values, color='#ff6b6b', linewidth=2,
        label=f"MCPS (12) | CAGR {stats_12['CAGR']}")

ax.set_title('MCPS Portfolio Size Analysis (15 vs 12 Stocks)', fontsize=16, color='white', pad=20)
...
out_path = repo_root / "mcps_12_stocks_analysis.png"
plt.tight_layout()
