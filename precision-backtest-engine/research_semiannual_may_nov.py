...
def run_semiannual_may_nov_backtest():
    # 1. Setup Data
    ...
    # 3. Define Semi-Annual Rebalance Dates (May 15 and Nov 15)
    all_dates = dh.get_all_dates()
    rebalance_dates = []
    for year in range(2017, 2027):
        for month in [5, 11]: # May and Nov
    ...
    # Ensure we start from May 2017 to be comparable
    start_date = "2017-05-15"
    end_date = "2026-02-05"
    
    # Filter rebalance dates to be within start/end (start date inclusive if it's a rebalance date)
    rebalance_dates = [d for d in rebalance_dates if d >= pd.Timestamp(start_date)]
    
    print(f"Starting Semi-Annual (May/Nov) Backtest...")
    ...
    print("\n" + "="*60)
    print("SEMI-ANNUAL REBALANCE (MAY/NOV) PERFORMANCE")
    print("="*60)
    ...
    print("="*60)
    print(f"{'Metric':<20} | {'May/Nov':<15} | {'Yearly (May)':<15} | {'Quarterly':<15}")
    print("-" * 65)
...
if __name__ == "__main__":
    run_semiannual_may_nov_backtest()
