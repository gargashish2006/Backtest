...
def run_8q_variation():
    ...
    print("Running Champion Strategy with 8-Quarter (2-Year) Lookback...")
    warnings.filterwarnings('ignore')

    # Champion Params with 8-Quarter Lookback
    strat = ContrarianBreadthStrategy(dh,
                                    ...
                                    shareholder_lookback_quarters=8) # VARIATION: 8 Quarters (2 Years)
    ...
    output_path = repo_root / "outputs/lookback_8q_nav.csv"
    nav_df.to_csv(output_path, index=False)
    
    stats = calculate_metrics(nav_df)
    
    print("\n" + "="*60)
    print("8-QUARTER LOOKBACK VARIATION PERFORMANCE")
    print("="*60)
    ...
    print("="*60)
    print(f"{'Metric':<20} | {'8-Quarter (2Y)':>15} | {'Champion (1Y)':>15}")
    print("-" * 55)
    ...
    if baseline_path.exists():
        ...
        plt.plot(champ_nav, label='Champion (1Y Lookback)', alpha=0.7)
        plt.plot(var_nav, label='Variation (2Y Lookback)', linewidth=2)
        plt.title('Strategy Comparison: Shareholder Lookback Period')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.savefig(repo_root / "outputs/lookback_8q_comparison.png")
        print(f"Chart saved to: {repo_root / 'outputs/lookback_8q_comparison.png'}")
