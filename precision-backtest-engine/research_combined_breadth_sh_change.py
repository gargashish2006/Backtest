...
def run_sh_change_ind_neutral_sim(dh, scale_factor=10000.0):
            ...
            if sh_trend.empty: 
                ...
                if not stocks_info:
                    ...
                    if final_pool.empty:
                        ...
                        # --- RANKING BY SH CHANGE % ---
                        # Max decrease on top = Most negative value first (ascending sort)
                        top50 = final_pool.sort_values('sh_change_pct', ascending=True).head(50)
                        
                        # --- INDUSTRY NEUTRAL WEIGHTING ---
                        industries_in_selection = top50['industry'].unique()
                        num_industries = len(industries_in_selection)
                        ind_weight = 1.0 / num_industries
                        
                        selection_with_weights = {}
                        for ind in industries_in_selection:
                            ind_stocks = top50[top50['industry'] == ind]['isin'].tolist()
                            stock_weight = ind_weight / len(ind_stocks)
                            for isin in ind_stocks:
...
def run_study():
    ...
    print("Running SH Change % Ranking Simulation (Top 50, Eq Industry)...")
    nav_model = run_sh_change_ind_neutral_sim(dh)
    ...
    # Previous Winner for Comparison (Industry Neutral Combined Breadth: 439.88)
    # We don't have the data here so we just print the new result
    
    print(f"Final NAV (SH Change % Model): {nav_model.iloc[-1]:.2f}")
    print(f"Final NAV (Top 1000 Bench): {nav_bench.iloc[-1]:.2f}")
    
    plt.figure(figsize=(10, 6))
    nav_model.plot(label='SH Change % Rank (Top 50 / Ind-Neutral)')
    nav_bench.plot(label='Top 1000 Benchmark', alpha=0.7)
    plt.title("SH Change % Ranking vs Top 1000 Bench")
    plt.ylabel("Value (Base 100)")
    plt.legend()
    plt.grid(True)
    plt.savefig(repo_root / "outputs/combined_breadth_sh_change_nav.png")
    print("\nPlot saved to: outputs/combined_breadth_sh_change_nav.png")
