...
class Top3GroupsConcentratedStrategy(ContrarianBreadthStrategy):
    """
    Variation:
    1. Select Top 3 Industry Groups.
    2. NO max_per_industry limit.
    ...
    def calculate_selection(self, date: pd.Timestamp) -> Dict[str, float]:
        ...
        for ind in ind_ranked['industry']:
            ...
            # VARIATION: NO LIMIT per industry
...
def run_top3_concentrated():
    ...
    print("Running Top 3 Groups CONCENTRATED Variation (No Limits)...")
    warnings.filterwarnings('ignore')

    strat = Top3GroupsConcentratedStrategy(dh,
                               num_stocks=15, 
    ...
    output_path = repo_root / "outputs/top3_conc_nav.csv"
    nav_df.to_csv(output_path, index=False)
    
    stats = calculate_metrics(nav_df)
    
    print("\n" + "="*60)
    print("TOP 3 CONCENTRATED VARIATION PERFORMANCE")
    print("="*60)
    ...
    print("="*60)
    print(f"{'Metric':<20} | {'Top 3 Conc':>15} | {'Champion':>15}")
    print("-" * 55)
    ...
    if baseline_path.exists():
        ...
        plt.plot(var_nav, label='Variation (Top 3 Conc)', linewidth=2)
        plt.title('Strategy Comparison: Top 3 Groups Concentrated vs Champion')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.savefig(repo_root / "outputs/top3_conc_comparison.png")
        print(f"Chart saved to: {repo_root / 'outputs/top3_conc_comparison.png'}")

if __name__ == "__main__":
    run_top3_concentrated()
