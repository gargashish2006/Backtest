...
def run_simulation(dh, group_pct, scale_factor=10000.0):
...
def run_yearly_sensitivity():
    ...
    for t in thresholds:
        label = f"Threshold {int(t*100)}%"
        print(f"Running Hierarchical 8Q with Group Threshold: {label}...")
        nav_series = run_simulation(dh, t)
        ...
        for y in years:
            ...
            if not y_start_data.empty and not y_end_data.empty:
                y_ret = (y_end_data.iloc[-1] / y_start_data.iloc[-1]) - 1
            else:
                y_ret = 0
            threshold_rets[y] = y_ret * 100
            
        yearly_results[label] = threshold_rets

    df = pd.DataFrame(yearly_results)
    print("\nTRI-TIER TERMINAL NAV COMPARISON:")
    for lbl, val in terminal_navs.items():
        print(f"{lbl}: {val:.2f}")

    print("\nYEARLY GROUP THRESHOLD SENSITIVITY (%):")
    print(df.round(2))
    
    # Plotting
    ax = df.plot(kind='bar', figsize=(12, 6), width=0.8)
    plt.title("Yearly Performance Sensitivity to Group Thresholds (Hierarchical 8Q)")
    plt.ylabel("Annual Return (%)")
    plt.xlabel("Year")
    plt.legend(title="Selection Threshold")
    plt.grid(axis='y', alpha=0.3)
    plt.tight_layout()
    
    plot_path = repo_root / "yearly_group_sensitivity.png"
    plt.savefig(plot_path)
    print(f"\nPlot saved to: {plot_path}")

if __name__ == "__main__":
    run_yearly_sensitivity()
