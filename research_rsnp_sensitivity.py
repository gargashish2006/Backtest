...
def run_rsnp_sensitivity():
    ...
    rsnp_thresholds = [0.34, 0.40, 0.50]
    results = []
    ...
    print(f"\nEvaluating RSNP Threshold Sensitivity (Fixed G:50% I:50%)...")
    for rsnp in rsnp_thresholds:
        ...
        print(f"RSNP:{rsnp} -> Return: {metrics['Absolute Return']} | DD: {metrics['Max Drawdown']}")

    df = pd.DataFrame(results)
    output_file = "/Users/shubhrakasana/Desktop/oleander/olgo_mobile/repos/Backtest/precision-backtest-engine/outputs/sensitivity_analysis_rsnp.csv"
    df.to_csv(output_file, index=False)
    print(f"\nAnalysis Complete. Saved to {output_file}")

if __name__ == "__main__":
    run_rsnp_sensitivity()
