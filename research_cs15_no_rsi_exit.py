...
class CS15NoRSIExitStrategy(CS15Strategy):
    """Subclass of CS15Strategy that disables the RSI < 39 exit rule."""
    def check_exits(self, current_date: pd.Timestamp, portfolio) -> List[str]:
        # Overriding to return empty list, effectively disabling daily RSI exits
        return []
...
def run_research():
    ...
    # 1. Setup Strategy (Disabled RSI exit)
    strategy = CS15NoRSIExitStrategy(dh)
    strategy.precompute_rsi(rdates)
    
    # 2. Setup Simulation (Same parameters as production)
    ...
    # 3. Run
    print("Running CS15 (No RSI Exit) Backtest...")
    ...
    print("\n" + "="*40)
    print("CS15 (NO RSI EXIT) PERFORMANCE")
    print("="*40)
