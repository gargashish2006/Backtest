...
def run_simulation(dh, num_stocks, max_per_ind, scale_factor=10000.0):
    ...
    # Setting very high limits for Unlimited selection
    ...
    for date in sim_dates:
        ...
        if date in rebalance_dates:
            ...
            if current_q_idx <= 8:
                ...
                portfolio_sizes.append({'date': date, 'count': len(targets)})
                invest_val = q_allot
                tranche_map = {}
    ...
    size_df = pd.DataFrame(portfolio_sizes).set_index('date')
    return nav_series, size_df

def run_unlimited_study():
    repo_root = Path("/Users/shubhrakasana/Desktop/oleander/olgo_mobile/repos/Backtest/precision-backtest-engine")
    dh = DataHandler(repo_root / "database/price_data_cleaned.parquet")
    dh.load_data()
    
    print("Running Unlimited Thematic Simulation (All Stocks in Top 35% Groups)...")
    nav_unlimited, sizes = run_simulation(dh, 9999, 9999)
    
    print(f"\nAverage Stocks per Selection: {sizes['count'].mean():.1f}")
    print(f"Max Stocks per Selection: {sizes['count'].max()}")
    print(f"Final NAV (Unlimited): {nav_unlimited.iloc[-1]:.2f}")
    
    # Extract Yearly
    years = [2020, 2021, 2022, 2023, 2024]
    y_rets = {}
    for y in years:
        y_start_data = nav_unlimited[nav_unlimited.index.year < y]
        y_end_data = nav_unlimited[nav_unlimited.index.year == y]
        if not y_start_data.empty and not y_end_data.empty:
            ret = (y_end_data.iloc[-1] / y_start_data.iloc[-1]) - 1
        else: ret = 0
        y_rets[y] = ret * 100
        
    print("\nYEARLY PERFORMANCE (Unlimited) (%):")
    ...
    nav_unlimited.plot()
    plt.title("Unlimited Thematic Portfolio: Top 35% Industry Groups (NAV)")
    plt.ylabel("Portfolio Value (Base 100)")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(repo_root / "unlimited_thematic_nav.png")
    
    print(f"\nPlot saved to: unlimited_thematic_nav.png")
