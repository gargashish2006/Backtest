"""
MCPS Ultra-Concentration Analysis (6 Stocks):
Compares MCPS (15), (12), (9) vs (6) stocks.
Configs:
- 15, 12, 9: max_per_industry=3
- 6: max_per_industry=2
All use Variant B, Group Top 50%, and cleaned price data.
"""
import pandas as pd
...
def run_variant(label, num_stocks, max_ind):
    print(f"Running {label} (num_stocks={num_stocks}, max_per_industry={max_ind})...")
    p = Portfolio(10_000_000)
    s = MCPSStrategy(dh, group_top_pct=0.50, num_stocks=num_stocks, max_per_industry=max_ind)
...
for label, n, m in variants:
    stats, nav = run_variant(label, n, m)
    results[label] = stats
...
# ── Print Table ───────────────────────────────────────────────────────────────
print("\n" + "=" * 110)
header = f"{'Metric':<20} | {'CS15':^12} | " + " | ".join(f"{l:^12}" for l, _, _ in variants)
print(header)
print("-" * 110)
for k in cs15_stats.keys():
    row = f"{k:<20} | {cs15_stats[k]:^12} | "
    row += " | ".join(f"{results[label][k]:^12}" for label, _, _ in variants)
    print(row)
print("=" * 110)
...
ax.set_title('MCPS Concentration Analysis (15, 12, 9, 6 Stocks)', fontsize=16, color='white', pad=20)
...
out_path = repo_root / "mcps_6_stocks_analysis.png"
plt.tight_layout()
