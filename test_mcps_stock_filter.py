"""
MCPS15 Sensitivity: Industry threshold (30/50/70%) + stock-level MCPS > 0 filter.
Stocks with negative MCPS change are excluded from final selection.
"""
import pandas as pd
...
class MCPS15StockFilter(MCPS15Strategy):
    ...
    2. Stock filter: exclude individual stocks with negative MCPS change
    """
    ...
    def _get_positive_mcps_isins(self, signal_date: pd.Timestamp) -> set:
        """Returns set of ISINs with positive MCPS change at signal_date."""
    ...
    def calculate_selection(self, date: pd.Timestamp):
        ...
        # Stocks with positive MCPS change only
        positive_mcps_isins = self._get_positive_mcps_isins(actual_signal_date)
        ...
        # *** Stock-level MCPS filter: exclude negative MCPS stocks ***
        if positive_mcps_isins:
            universe = universe[universe['isin'].isin(positive_mcps_isins)]
...
def run_variant(label, min_pct):
    ...
    s = MCPS15StockFilter(dh, min_mcps_pct=min_pct)
...
# ── Run all variants ──────────────────────────────────────────────────────────
variants = [
    ("MCPS15 + Stock Filter (no ind. threshold)", 0.0),
    ("MCPS15 + Stock Filter (>= 30%)",            0.30),
    ("MCPS15 + Stock Filter (>= 50%)",            0.50),
    ("MCPS15 + Stock Filter (>= 70%)",            0.70),
]
results, navs = {}, {}
...
out_path = repo_root / "mcps15_stock_filter_sensitivity.png"
plt.tight_layout()
