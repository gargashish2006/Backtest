...
def run_top100_strategy():
    ...
    print("Running Champion Strategy on TOP 100 UNIVERSE...")
    warnings.filterwarnings('ignore')

    # Champion Params, but Universe Size = 100
    strat = ContrarianBreadthStrategy(dh,
                                    num_stocks=15, 
                                    max_per_industry=3,
                                    universe_size=100,  # VARIATION: Top 100 Only
    ...
    output_path = repo_root / "outputs/top100_nav.csv"
    nav_df.to_csv(output_path, index=False)
    
    stats = calculate_metrics(nav_df)
    
    print("\n" + "="*60)
    print("TOP 100 UNIVERSE STRATEGY PERFORMANCE")
    print("="*60)
    ...
    # Comparison
    print("="*60)
    print("COMPARISON VS CHAMPION BASELINE (Universe 1000)")
    print("="*60)
    print(f"{'Metric':<20} | {'Top 100 Univ':>15} | {'Champion (1000)':>15}")
    print("-" * 55)
    ...
    if baseline_path.exists():
        ...
        plt.plot(champ_nav, label='Champion (Universe 1000)', alpha=0.7)
        plt.plot(var_nav, label='Variation (Top 100 Only)', linewidth=2)
        plt.title('Strategy Comparison: Top 100 Universe vs Top 1000 Universe')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.savefig(repo_root / "outputs/top100_comparison.png")
        print(f"Chart saved to: {repo_root / 'outputs/top100_comparison.png'}")
