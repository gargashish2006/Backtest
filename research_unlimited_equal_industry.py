...
class UnlimitedEqualIndustryStrategy(ContrarianBreadthStrategy):
    """
    Variation:
    1. Select ALL stocks that qualify (No max_per_industry, No global num_stocks limit)
    2. Equal Industry Weighting: Each qualified Industry gets equal allocation.
       Stocks within the industry share that allocation equally.
    """
    def calculate_selection(self, date: pd.Timestamp) -> Dict[str, float]:
        ...
        lookback_dates = [d for d in all_dates if d <= (actual_calc_date - pd.Timedelta(days=self.price_lookback_days))]
        if not lookback_dates: return {}
        ...
        # Relax universe size to capture "ALL" potential candidates (e.g., top 2000 implies broad market)
        universe = metrics.sort_values('mc', ascending=False).head(2000)
        ...
        final_industries = ind_ranked['industry'].tolist()
        
        # 5. Stock Selection (RSI Limit) - Select ALL qualifying in final industries
        universe = universe[universe['isin'].map(self.dh.isin_to_industry).isin(final_industries)]
        ...
        # 6. Weighting: Equal Industry Weight
        # Identify industries that actually have at least 1 stock selected
        universe['industry'] = universe['isin'].map(self.dh.isin_to_industry)
        active_industries = universe['industry'].unique().tolist()
        
        if not active_industries: return {}
        
        num_active_inds = len(active_industries)
        weight_per_industry = 1.0 / num_active_inds
        
        weights = {}
        for ind in active_industries:
            stocks_in_ind = universe[universe['industry'] == ind]['isin'].tolist()
            num_stocks = len(stocks_in_ind)
            weight_per_stock = weight_per_industry / num_stocks
            for isin in stocks_in_ind:
                weights[isin] = weight_per_stock
                
        return weights

def run_unlimited_equal_industry():
    ...
    print("Running Unlimited Equal Industry Variation (No limits, Equal Industry Weights)...")
    warnings.filterwarnings('ignore')

    strat = UnlimitedEqualIndustryStrategy(dh, num_stocks=9999) # Dummy num_stocks, logic inside handles it
    ...
    output_path = repo_root / "outputs/unlimited_equal_ind_nav.csv"
    nav_df.to_csv(output_path, index=False)
    
    stats = calculate_metrics(nav_df)
    
    print("\n" + "="*60)
    print("UNLIMITED EQUAL INDUSTRY VARIATION PERFORMANCE")
    print("="*60)
    ...
    print("="*60)
    print(f"{'Metric':<20} | {'Unlimited':>15} | {'Champion':>15}")
    print("-" * 55)
    ...
    if baseline_path.exists():
        ...
        plt.plot(champ_nav, label='Champion (15 Stocks)', alpha=0.7)
        plt.plot(var_nav, label='Variation (Unlimited, Eq Ind)', linewidth=2)
        plt.title('Strategy Comparison: Unlimited Selection vs Champion')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.savefig(repo_root / "outputs/unlimited_equal_ind_comparison.png")
        print(f"Chart saved to: {repo_root / 'outputs/unlimited_equal_ind_comparison.png'}")

if __name__ == "__main__":
    run_unlimited_equal_industry()
