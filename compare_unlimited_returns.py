...
def compare_rebalances():
    ...
    var_path = repo_root / "outputs/unlimited_equal_ind_nav.csv"
    ...
    merged = pd.merge(champ_df, var_df, on='date', suffixes=('_champ', '_unlim'))
    ...
    print("\n" + "="*80)
    print(f"{'Period End':<15} | {'Champ (1Y)':>12} | {'Unlimited':>12} | {'Diff':>12}")
    print("-" * 80)
    
    for _, row in res_df.iterrows():
        print(f"{row['Period End']:<15} | {row['Champion (1Y)']:.2%}       | {row['Unlimited Eq Ind']:.2%}       | {row['Diff (Champ - Var)']:.2%}")
    ...
    out_path = repo_root / "outputs/rebalance_comparison_unlimited.csv"
