...
def run_gradual_balanced():
    ...
    # Strategy Config: Top 10 Industries, 2 stocks each (Total 20)
    strategy = StructuralAlphaStrategy(dh, num_stocks=20, max_per_industry=2)
    ...
    print(f"Starting Balanced Gradual Entry (20 stocks, Top 10 Ind, 25% x 4) from {start_date.date()}...")
    ...
    plt.plot(df_nav['nav'] / scale_factor, label="Balanced Strategy (20 stocks, Top 10 Ind, 2-Max)")
    plt.plot(df_bench['nav'] / scale_factor, label="Staggered Benchmark (Static)", alpha=0.7, color='gray')
    plt.title(f"Balanced Gradual Entry: Top 10 Industries (2-Max)\nStrategy: {final_nav:.1f} | Bench: {final_bench:.1f}")
    plt.ylabel("Portfolio Value (Base 100)")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(repo_root / "gradual_balanced_results.png")
    
    print(f"\nBALANCED GRADUAL ENTRY COMPLETE")
    print(f"Final Strategy Value: {final_nav:.2f}")
    print(f"Final Benchmark Value: {final_bench:.2f}")

if __name__ == "__main__":
    run_gradual_balanced()
