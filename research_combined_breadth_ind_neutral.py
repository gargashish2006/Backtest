...
def run_combined_breadth_ind_neutral_sim(dh, scale_factor=10000.0):
            ...
            if sh_trend.empty: 
                ...
                if not stocks_info:
                    ...
                    if final_pool.empty:
                        ...
                        # Select Top 50 stocks by score
                        top50 = final_pool.sort_values('combined_score', ascending=False).head(50)
                        
                        # --- INDUSTRY NEUTRAL WEIGHTING ---
                        # 1. Identify distinct industries
...
def run_study():
    ...
    print("Running Combined Breadth Industry Neutral Simulation (Top 50, Eq Industry)...")
    nav_model = run_combined_breadth_ind_neutral_sim(dh)
    ...
    print(f"Final NAV (Industry Neutral Model): {nav_model.iloc[-1]:.2f}")
    print(f"Final NAV (Top 1000 Bench): {nav_bench.iloc[-1]:.2f}")
    
    plt.figure(figsize=(10, 6))
    nav_model.plot(label='Combined Breadth (Top 50 / Ind-Neutral)')
    nav_bench.plot(label='Top 1000 Benchmark', alpha=0.7)
    plt.title("Combined Breadth Industry Neutral vs Top 1000 Bench")
    plt.ylabel("Value (Base 100)")
    plt.legend()
    plt.grid(True)
    plt.savefig(repo_root / "outputs/combined_breadth_ind_neutral_nav.png")
    print("\nPlot saved to: outputs/combined_breadth_ind_neutral_nav.png")
