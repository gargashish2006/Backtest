"""
MCPS Portfolio Size Analysis (9 Stocks):
Compares MCPS (15 stocks) vs MCPS (12 stocks) vs MCPS (9 stocks).
All use max_per_industry=3, Group Top 50%, and cleaned price data.
"""
import pandas as pd
...
from strategies.mcps15_strategy import MCPSStrategy # Renamed class
...
def run_variant(label, num_stocks):
    ...
    s = MCPSStrategy(dh, group_top_pct=0.50, num_stocks=num_stocks, max_per_industry=3)
...
# ── Print Table ───────────────────────────────────────────────────────────────
print("\n" + "=" * 100)
header = f"{'Metric':<20} | {'CS15 (15)':^12} | " + " | ".join(f"{l:^12}" for l in nav_history.keys())
print(header)
print("-" * 100)
...
ax.set_title('MCPS Portfolio Size Analysis (15 vs 12 vs 9 Stocks)', fontsize=16, color='white', pad=20)
...
out_path = repo_root / "mcps_9_stocks_analysis.png"
plt.tight_layout()
