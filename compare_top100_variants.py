"""
Frictionless comparison of 5 Top-100 variants:
1. Baseline (Index)
2. SH Decrease (1Q)
3. SH Decrease (4Q)
4. MCPS Increase (1Q)
...
class Top100VariantStrategy:
    ...
    def _get_q_labels(self, date, lag):
        ...
        # Simple lag logic for comparison
        # (For 1Q vs 4Q)
        # Note: This is an approximation for the demo script
        months = [2, 5, 8, 11]
    ...
    def calculate_selection(self, date):
        # 1. Get Top 100 Universe by Market Cap
        top_100 = self.dh.get_universe(date, size=100)
        if top_100.empty: return {}
        
        if self.mode == 'baseline':
            isins = top_100['isin'].tolist()
        else:
            ...
            merged = pd.merge(top_100, curr_sh, on='isin')
            merged = pd.merge(merged, prev_sh, on='isin')
            
            if self.mode.startswith('SH_Dec'):
                # SH Decrease: curr < prev
                selected = merged[merged['curr'] < merged['prev']]
            elif self.mode.startswith('MCPS_Incr'):
                # MCPS Increase: (mc / curr) > (mc_prev / prev)
                # We'll use the current 'mc' vs reported mc_prev if we had it, 
                # but let's approximate with (current_mc / curr_sh) vs (prev_mc / prev_sh)
                # Since we don't store historical SH_MC easily in this step, 
                # let's assume we want Price_t / SH_t > Price_{t-lag} / SH_{t-lag}
                # To be precise, let's just use the current MC and SH from the merge
                # Actually, MC change is driven by Price. So (Price_t / SH_t) > (Price_{t-lag} / SH_{t-lag})
                # Let's get price history
...
def run_sim(mode):
    ...
    strat = Top100VariantStrategy(dh, mode)
    port = Portfolio(10000000)
    fee_model = FeeModel(0, 0) # Frictionless
    tax_man = TaxManager(0, 0) # Frictionless
...
ax.set_title("Top 100 Selection Variants: SH cleaning vs. MCPS Growth", color='white', fontsize=18)
ax.set_ylabel("Normalized NAV", color='white')
ax.legend(facecolor='#1a1a2e', labelcolor='white')
ax.grid(True, color='#222222')
ax.tick_params(colors='white')

out_path = repo_root / "top100_variants_comparison.png"
plt.tight_layout()
