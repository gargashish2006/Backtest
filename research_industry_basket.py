...
def run_industry_basket_simulation(dh, scale_factor=10000.0):
    ...
    for date in sim_dates:
        ...
        if date in rebalance_dates:
            ...
            group_breadth = sh_trend.groupby('isin').first() # Mocking group mapping
            # We need actual group/industry mapping from DH
            stocks_with_info = []
            ...
            # 3. Top 10 Industries within Top Groups
            ...
            top_10_industries = industry_stats.head(10).index.tolist()
            ...
            for ind in top_10_industries:
                ...
                if top_3:
                    weight_per_stock = 0.10 / len(top_3) # 1/10 split equally among selected stocks
                    for isin in top_3:
...
def run_industry_basket_study():
    repo_root = Path("/Users/shubhrakasana/Desktop/oleander/olgo_mobile/repos/Backtest/precision-backtest-engine")
    dh = DataHandler(repo_root / "database/price_data_cleaned.parquet")
    dh.load_data()
    
    print("Running Industry Basket Simulation (10 Ind / 3 Stocks / Industry Weighted)...")
    nav_basket = run_industry_basket_simulation(dh)
    
    print(f"Final NAV (Fixed Industry Basket): {nav_basket.iloc[-1]:.2f}")
    ...
    nav_basket.plot()
    plt.title("Fixed Industry Basket: Top 35% Groups -> 10 Ind -> 3 Stock (High MC)")
    plt.ylabel("NAV (Base 100)")
    plt.grid(True)
    plt.savefig(repo_root / "fixed_industry_basket_nav.png")
    print(f"\nPlot saved to: fixed_industry_basket_nav.png")

if __name__ == "__main__":
    run_industry_basket_study()
