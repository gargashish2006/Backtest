...
def run_industry_qualification_research():
    ...
    print(f"\nRunning Industry Qualification Research ({start_date} to {end_date})...")
    ...
    # 5. Summary & Save
    print("\n" + "="*80)
    print("INDUSTRY QUALIFICATION RESULTS Summary")
    print("="*80)
    ...
    for name, data in results.items():
        ...
        # Save NAV
        data['nav'].to_csv(repo_root / f"outputs/ind_qual_{name}_nav.csv", index=False)
    
    pd.DataFrame(summary_data).to_csv(repo_root / "outputs/ind_qual_results.csv", index=False)

if __name__ == "__main__":
    run_industry_qualification_research()
