"""
Industry-Level Shareholder Analysis with Benchmark Comparison

Analyzes INDUSTRIES based on percentage of stocks with increasing shareholders.
Calculates forward returns from INDUSTRY BENCHMARKS (not individual stocks).

Rebalance Dates: Mid-Feb, Mid-May, Mid-Aug, Mid-Nov (quarterly)
Strategy: Select top/bottom 5 industries by % increasing shareholders
Returns: Calculated from industry benchmark performance
...
BENCHMARK_DIR = BASE_DIR / 'analysis' / 'outputs' / 'benchmarks' / 'industries'
...
class IndustryShareholderAnalyzer:
    """Analyzer for industry performance based on shareholder trends using benchmarks."""
    
    def __init__(self):
        """Initialize the analyzer and load data."""
        print("=" * 80)
        print("INDUSTRY SHAREHOLDER BENCHMARK ANALYSIS")
        print("=" * 80)
        ...
        self.min_stocks_per_industry = 5  # Minimum stocks required for industry selection
        self.top_n_industries = 10  # Select top/bottom 10 industries
    ...
    def load_data(self):
        ...
        # Filter out 'Not Available' category
        self.industry_df = self.industry_df[self.industry_df['industry'] != 'Not Available']
        print(f"    Loaded {len(self.industry_df):,} industry mappings (excluding 'Not Available')")
        
        # Create ISIN to industry mapping
        self.isin_to_industry = dict(zip(
            self.industry_df['isin'],
            self.industry_df['industry']
        ))
        ...
        benchmark_dfs = []
        for industry_dir in BENCHMARK_DIR.iterdir():
            if industry_dir.is_dir():
                timeseries_file = industry_dir / 'timeseries.csv'
                if timeseries_file.exists():
                    df = pd.read_csv(timeseries_file)
                    df['industry'] = industry_dir.name.replace('_', ' ')
                    benchmark_dfs.append(df[['industry', 'date', 'index_value']])
        ...
        self.benchmark_df = self.benchmark_df.sort_values(['industry', 'date'])
        print(f"    Loaded {len(self.benchmark_df):,} benchmark records")
        
        # Create benchmark lookup: {industry: {date: index_value}}
        print("  - Creating benchmark lookup dictionary...")
        self.benchmark_lookup = {}
        for industry in self.benchmark_df['industry'].unique():
            industry_data = self.benchmark_df[self.benchmark_df['industry'] == industry]
            self.benchmark_lookup[industry] = dict(zip(
                industry_data['date'],
                industry_data['index_value']
            ))
        print(f"    Created lookup for {len(self.benchmark_lookup)} industries")
        
        # Get unique industries
        self.industries = sorted([i for i in self.benchmark_df['industry'].unique() 
                                 if i in self.industry_df['industry'].values])
        print(f"    Found {len(self.industries)} industries for analysis")
    ...
    def get_rebalance_dates(self):
        ...
        min_date = max(min_date, self.benchmark_min_date - pd.DateOffset(months=6))
    ...
    def calculate_industry_metrics_at_rebalance(
        self, 
        rebalance_date: pd.Timestamp,
        lookback_quarters: int
    ):
        """
        Calculate shareholder metrics for each industry at rebalance date.
        ...
        # Add industry
        merged['industry'] = merged['isin'].map(self.isin_to_industry)
        ...
        for industry in merged['industry'].unique():
            ...
            num_stocks = len(industry_data)
            
            if num_stocks < self.min_stocks_per_industry:
                continue
            
            num_increasing = industry_data['is_increasing'].sum()
            num_decreasing = industry_data['is_decreasing'].sum()
    ...
    def select_top_bottom_industries(self, industry_metrics):
        """
        Select top and bottom N industries by % increasing shareholders.
        
        Returns:
            (top_industries, bottom_industries) - Lists of industry names
        """
        ...
        # Top N industries (highest % increasing)
        top_industries = [industry for industry, _ in sorted_industries[:self.top_n_industries]]
        
        # Bottom N industries (lowest % increasing = highest % decreasing)
        bottom_industries = [industry for industry, _ in sorted_industries[-self.top_n_industries:]]
    ...
    def calculate_benchmark_return(
        ...
        """
        Calculate return for an industry using benchmark values.
        ...
        if industry not in self.benchmark_lookup:
            return np.nan
        
        benchmark_dict = self.benchmark_lookup[industry]
    ...
    def run_comprehensive_analysis(self):
        ...
        print(f"  - Min stocks per industry: {self.min_stocks_per_industry}")
        print(f"  - Top/Bottom N industries: {self.top_n_industries}")
    ...
    def analyze_combination(
        ...
        for idx, rebalance_date in enumerate(valid_rebalance_dates, 1):
            ...
            industry_metrics = self.calculate_industry_metrics_at_rebalance(
            ...
            top_industries, bottom_industries = self.select_top_bottom_industries(
                industry_metrics
            )
            
            # Calculate returns for top industries (increasing shareholders)
            for industry in top_industries:
                ret = self.calculate_benchmark_return(
                    industry, rebalance_date, holding_days
                )
                ...
                if not pd.isna(ret):
                    results.append({
                        ...
                        'pct_increasing': industry_metrics[industry]['pct_increasing'],
                        'num_stocks': industry_metrics[industry]['num_stocks'],
                        'forward_return': ret
                    })
            
            # Calculate returns for bottom industries (decreasing shareholders)
    ...
    def generate_summary(self):
        ...
        summary_file = REPORTS_DIR / f'industry_summary_{timestamp}.csv'
        self.summary_df.to_csv(summary_file, index=False)
        print(f"\n✓ Saved summary to {summary_file}")
        
        # Save detailed results
        detailed_file = REPORTS_DIR / f'industry_detailed_{timestamp}.csv'
    ...
    def plot_heatmaps(self):
        ...
        timestamp = datetime.now().strftime('%Y%m%d')
        chart_file = CHARTS_DIR / f'industry_heatmaps_{timestamp}.png'
        plt.savefig(chart_file, dpi=300, bbox_inches='tight')
        print(f"✓ Saved heatmaps to {chart_file}")
        plt.close()
    
    def plot_comparison_charts(self):
        """Create comparison charts between top and bottom industries."""
        print("\n" + "=" * 80)
        ...
        timestamp = datetime.now().strftime('%Y%m%d')
        chart_file = CHARTS_DIR / f'industry_comparison_{timestamp}.png'
        plt.savefig(chart_file, dpi=300, bbox_inches='tight')
...
def main():
    ...
    print("\n✓ Industry shareholder benchmark analysis complete!")
