#!/usr/bin/env python
"""
Combined Shareholding Analysis Plot - 2 Quarter Changes

Plots both:
1. % of stocks with increasing market cap per shareholder (over 2 quarters)
2. % of stocks with increasing number of shareholders (over 2 quarters)
...
class TwoQuarterChangeAnalyzer:
    """Analyze 2-quarter changes in shareholding patterns"""
    ...
    def analyze_two_quarter_changes(self):
        """
        Calculate 2-quarter changes for both metrics
        
        Returns:
            DataFrame with both metrics
        """
        print("\nAnalyzing 2-quarter changes...")
        ...
        print("\n  Combining data and calculating 2-quarter changes...")
        ...
        # Calculate values 2 quarters ago using shift(2)
        all_data['prev_2q_shareholders'] = all_data.groupby('isin')['total_shareholders'].shift(2)
        all_data['prev_2q_mcap_per_sh'] = all_data.groupby('isin')['market_cap_per_shareholder'].shift(2)
        
        # Calculate changes
        all_data['shareholders_2q_change'] = all_data['total_shareholders'] - all_data['prev_2q_shareholders']
        all_data['mcap_per_sh_2q_change'] = all_data['market_cap_per_shareholder'] - all_data['prev_2q_mcap_per_sh']
        
        # Determine if increasing
        all_data['shareholders_increasing'] = all_data['shareholders_2q_change'] > 0
        all_data['mcap_per_sh_increasing'] = all_data['mcap_per_sh_2q_change'] > 0
        
        # Group by quarter and calculate percentages
        results = []
        
        for quarter_date in sorted(unique_quarters[2:]):  # Skip first 2 quarters (no 2-quarter-ago data)
            quarter_data = all_data[all_data['quarter_date'] == quarter_date]
            
            # Only consider stocks where we have 2-quarter-ago data
            comparable = quarter_data.dropna(subset=['prev_2q_shareholders', 'prev_2q_mcap_per_sh'])
            ...
            if len(comparable) > 0:
                ...
                results.append({
                    ...
                    'avg_shareholders_change': comparable['shareholders_2q_change'].mean(),
                    'avg_mcap_per_sh_change': comparable['mcap_per_sh_2q_change'].mean()
                })
        
        results_df = pd.DataFrame(results)
        
        print(f"\n✅ Calculated 2-quarter changes for {len(results_df)} quarters")
        
        return results_df
    
    def plot_results(self, results_df):
        """Plot the combined 2-quarter analysis"""
        ...
        ax1.plot(results_df['quarter_date'], results_df['pct_mcap_per_sh_increasing'],
                ...
                markersize=6,
                color='#2E86AB',
                label='% Stocks: Increasing Market Cap per Shareholder (2Q)',
                alpha=0.8)
        ...
        ax1.plot(results_df['quarter_date'], results_df['pct_shareholders_increasing'],
                ...
                markersize=6,
                color='#E63946',
                label='% Stocks: Increasing Number of Shareholders (2Q)',
                alpha=0.8)
        ...
        ax1.set_title('Shareholding Pattern Comparison (2-Quarter Changes): Market Cap per Shareholder vs Number of Shareholders',
        ...
        ax2.set_title('Concentration vs Participation (2Q Change): Difference Between Metrics\n(Positive = More concentration, Negative = More participation)',
                     fontsize=13, fontweight='bold', pad=15)
        ...
        # Add annotation
        ax2.text(0.02, 0.97, 
                 'Green: Wealth concentrating (fewer but wealthier shareholders over 2Q)\n'
                 'Red: Broader participation (more shareholders joining over 2Q)',
        ...
        ax3.scatter(recent['pct_shareholders_increasing'], recent['pct_mcap_per_sh_increasing'],
                   s=100,
                   alpha=0.6,
                   c=range(len(recent)),
                   cmap='viridis',
        ...
        # Add trend line
        z = np.polyfit(recent['pct_shareholders_increasing'], recent['pct_mcap_per_sh_increasing'], 1)
        p = np.poly1d(z)
        x_trend = np.linspace(recent['pct_shareholders_increasing'].min(), 
                             recent['pct_shareholders_increasing'].max(), 100)
        ...
        ax3.set_xlabel('% Stocks: Increasing Number of Shareholders (2Q)', fontsize=11, fontweight='bold')
        ax3.set_ylabel('% Stocks: Increasing Market Cap per Shareholder (2Q)', fontsize=11, fontweight='bold')
        ax3.set_title('Correlation Analysis (Last 20 Quarters, 2Q Changes)\nDarker points = More recent',
                     fontsize=13, fontweight='bold', pad=15)
        ax3.set_xlim(0, 100)
        ax3.set_ylim(0, 100)
        ...
        output_path = output_dir / 'combined_shareholding_2q_change_analysis_plot.png'
    ...
    def save_results(self, results_df):
        ...
        output_path = output_dir / 'combined_shareholding_2q_change_analysis.csv'
...
def main():
    """Main execution"""
    print("="*80)
    print("COMBINED SHAREHOLDING ANALYSIS - 2 QUARTER CHANGES")
    print("="*80)
    
    analyzer = TwoQuarterChangeAnalyzer()
    
    # Analyze 2-quarter changes
    results_df = analyzer.analyze_two_quarter_changes()
    ...
    print(f"\nMarket Cap per Shareholder (2Q Change):")
    ...
    print(f"\nNumber of Shareholders (2Q Change):")
