...
def export_feb_2026_im_ranking():
    ...
    print("\n" + "="*60)
    print("TOP 10 INDUSTRIES - INDUSTRY MOMENTUM (FEB 2026)")
    print("="*60)
    print(top_10.to_string(index=False))
    print("="*60)
    
    # Save to outputs
    out = Path(project_root) / 'strategies/industry_momentum/outputs' / 'feb_2026_industry_ranking_im.csv'
    out.parent.mkdir(parents=True, exist_ok=True)
    df_report.to_csv(out, index=False)
    print(f"Full report saved to: {out}")

if __name__ == "__main__":
    export_feb_2026_im_ranking()
