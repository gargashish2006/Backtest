#!/usr/bin/env python
"""
Promoter + FII Double Conviction Strategy

Logic:
1. Filter: Stocks where BOTH Promoter and FII percentage increased over a 4-quarter lookback.
...
class PromoterFIIIncreaseRS:
    def __init__(self):
        print("="*100)
        print("PROMOTER + FII DOUBLE CONVICTION + INDUSTRY RS (20 STOCKS)")
        print("="*100)
    ...
    def preprocess_data(self):
        ...
        # Promoter + FII increase over 4 quarters
        # Double check column names from research step: promoter_holding_pct, fii_holding_pct
        self.shp_with_info['prev_promoter'] = self.shp_with_info.groupby('isin')['promoter_holding_pct'].shift(self.LOOKBACK_QUARTERS)
        self.shp_with_info['prev_fii'] = self.shp_with_info.groupby('isin')['fii_holding_pct'].shift(self.LOOKBACK_QUARTERS)
        
        self.shp_with_info['promoter_buying'] = (self.shp_with_info['promoter_holding_pct'] - self.shp_with_info['prev_promoter']) > 0
        self.shp_with_info['fii_buying'] = (self.shp_with_info['fii_holding_pct'] - self.shp_with_info['prev_fii']) > 0
        self.shp_with_info['double_conviction'] = self.shp_with_info['promoter_buying'] & self.shp_with_info['fii_buying']
    ...
    def calculate_selection(self, date):
        ...
        # Only stocks with BOTH promoter and FII buying
    ...
    def run(self):
        ...
        print(f"\nPROMOTER + FII RS RETURN: {ret:.2f}%")
        p = self.base_path / 'strategies' / 'new_research' / 'outputs'
        p.mkdir(parents=True, exist_ok=True)
        res.to_csv(p / f'promoter_fii_increase_rs_equity_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv', index=False)

if __name__ == "__main__":
    PromoterFIIIncreaseRS().run()
