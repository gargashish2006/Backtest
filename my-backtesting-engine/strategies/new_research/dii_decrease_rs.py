#!/usr/bin/env python
"""
DII Decrease Contrarian Strategy

Logic:
1. Filter: Stocks where DII percentage DECREASED over a 4-quarter lookback. (Contrarian)
...
class DIIDecreaseRS:
    def __init__(self):
        print("="*100)
        print("DII DECREASE CONTRARIAN + INDUSTRY RS (20 STOCKS)")
        print("="*100)
    ...
    def preprocess_data(self):
        ...
        # DII decrease over 4 quarters
        self.shp_with_info['prev_dii'] = self.shp_with_info.groupby('isin')['dii_holding_pct'].shift(self.LOOKBACK_QUARTERS)
        self.shp_with_info['dii_decreasing'] = (self.shp_with_info['dii_holding_pct'] - self.shp_with_info['prev_dii']) < 0
    ...
    def calculate_selection(self, date):
        ...
        # 2. DII FILTER (STRICTLY TOP 1000)
        ...
        # Only stocks with decreasing DII holdings
        candidate_data = recent[recent['dii_decreasing'] == True]
    ...
    def run(self):
        ...
        print(f"\nDII DECREASE RS RETURN: {ret:.2f}%")
        p = self.base_path / 'strategies' / 'new_research' / 'outputs'
        p.mkdir(parents=True, exist_ok=True)
        res.to_csv(p / f'dii_decrease_rs_equity_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv', index=False)
