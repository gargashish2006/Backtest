#!/usr/bin/env python
"""
Top 100 Blue Chip Sentiment Strategy

Logic:
1. Universe: Strictly TOP 100 stocks by Market Cap on rebalance date.
2. Filter: Stocks where total shareholders DECREASED over a 4-quarter lookback.
3. Industry Filter: Parent Industry must have Positive Relative Strength (RS > 0).
   Industry RS is calculated using only Top 1000 members of that industry.
4. Selection: Top 20 stocks (or all qualifying if < 20).
...
class Top100SentimentRS:
    def __init__(self):
        print("=" * 100)
        print("TOP 100 BLUE CHIP SENTIMENT + INDUSTRY RS (MAX 20 STOCKS)")
        print("=" * 100)
    ...
    def preprocess_data(self):
        ...
        # Shareholder decrease over 4 quarters
        self.shp_with_info['prev_shareholders'] = self.shp_with_info.groupby('isin')['total_shareholders'].shift(
            self.LOOKBACK_QUARTERS)
        self.shp_with_info['decreasing'] = (self.shp_with_info['total_shareholders'] - self.shp_with_info[
            'prev_shareholders']) < 0
    ...
    def calculate_selection(self, date):
        # 1. TOP 100 UNIVERSE ON THIS DATE
        ...
        # We still need Top 1000 for RS calculation
        top_1000_isins = p_slice.dropna(subset=['mc']).sort_values('mc', ascending=False).head(1000)['isin'].tolist()
        top_100_isins = top_1000_isins[:100]

        # 2. SENTIMENT FILTER (STRICTLY TOP 100)
        ...
        recent = self.shp_with_info[(self.shp_with_info['quarter_date'] >= cut) & (
                    self.shp_with_info['quarter_date'] <= date) & (
                                        self.shp_with_info['isin'].isin(top_100_isins))].copy()
        recent = recent.sort_values('quarter_date').groupby('isin').last().reset_index()

        # Only stocks with decreasing shareholders
        candidate_data = recent[recent['decreasing'] == True]
        if candidate_data.empty: return []

        # 3. INDUSTRY RS RANKING (STRICTLY TOP 1000 MEMBERS for RS computation)
        ...
        # 4. FILTER FOR POSITIVE RS + RANK
        candidate_data['ind_rs'] = candidate_data['industry'].map(ind_rs)
        candidate_data = candidate_data[candidate_data['ind_rs'] > 0]
        if candidate_data.empty: return []
        
        candidate_data = candidate_data.merge(p_slice[['isin', 'mc']], on='isin', how='left')
        selected_isins = candidate_data.sort_values(by=['ind_rs', 'mc'], ascending=False)['isin'].tolist()
    ...
    def run(self):
        ...
        for i, date in enumerate(dates):
            ...
            # NEW ALLOCATION LOGIC: Equal Weighted with 10% Cap
            num_stocks = len(stocks)
            # Target weight = 1.0 / N, but capped at 0.10. Total invested max 0.95
            target_weight = min(0.95 / num_stocks, 0.10)
            target_per_stock = curr_val * target_weight
        ...
        print(f"\nTOP 100 SENTIMENT RS RETURN: {ret:.2f}%")
        p = self.base_path / 'strategies' / 'new_research' / 'outputs'
        p.mkdir(parents=True, exist_ok=True)
        res.to_csv(p / f'top_100_sentiment_rs_equity_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv', index=False)


if __name__ == "__main__":
    Top100SentimentRS().run()
