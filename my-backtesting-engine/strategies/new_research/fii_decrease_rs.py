#!/usr/bin/env python
"""
FII Decrease Contrarian Strategy

Logic:
1. Filter: Stocks where FII percentage DECREASED over a 4-quarter lookback. (Contrarian)
...
class FIIDecreaseRS:
    def __init__(self):
        print("=" * 100)
        print("FII DECREASE CONTRARIAN + INDUSTRY RS (20 STOCKS)")
        print("=" * 100)
    ...
    def preprocess_data(self):
        ...
        # FII decrease over 4 quarters
        self.shp_with_info['prev_fii'] = self.shp_with_info.groupby('isin')['fii_holding_pct'].shift(
            self.LOOKBACK_QUARTERS)
        self.shp_with_info['fii_decreasing'] = (self.shp_with_info['fii_holding_pct'] - self.shp_with_info[
            'prev_fii']) < 0
    ...
    def calculate_selection(self, date):
        ...
        # 2. FII FILTER (STRICTLY TOP 1000)
        ...
        # Only stocks with decreasing FII holdings
        candidate_data = recent[recent['fii_decreasing'] == True]
    ...
    def run(self):
        ...
        print(f"\nFII DECREASE RS RETURN: {ret:.2f}%")
        p = self.base_path / 'strategies' / 'new_research' / 'outputs'
        p.mkdir(parents=True, exist_ok=True)
        res.to_csv(p / f'fii_decrease_rs_equity_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv', index=False)
