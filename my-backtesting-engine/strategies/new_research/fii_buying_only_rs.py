#!/usr/bin/env python
"""
FII Buying ONLY Strategy

Logic:
1. Filter: Stocks where FII percentage increased over a 4-quarter lookback.
...
class FIIBuyingOnlyRS:
    def __init__(self):
        print("="*100)
        print("FII BUYING ONLY + INDUSTRY RS (20 STOCKS)")
        print("="*100)
    ...
    def calculate_selection(self, date):
        ...
        # Only stocks with increasing FII holdings
        candidate_data = recent[recent['fii_buying'] == True]
    ...
    def run(self):
        ...
        print(f"\nFII RS RETURN: {ret:.2f}%")
        p = self.base_path / 'strategies' / 'new_research' / 'outputs'
        p.mkdir(parents=True, exist_ok=True)
        res.to_csv(p / f'fii_buying_only_rs_equity_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv', index=False)
