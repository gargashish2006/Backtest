#!/usr/bin/env python
"""
Promoter Inc + Shareholder Dec (Double Conviction) Strategy

Logic:
1. Filter: Stocks where:
   - Promoter percentage INCREASED over a 4-quarter lookback.
   - Total Shareholders DECREASED over a 4-quarter lookback.
2. Universe: Strictly TOP 1000 stocks by Market Cap on rebalance date.
3. Ranking: Rank qualifying stocks by their parent Industry's Relative Strength (RS vs Top 1000).
   Industry RS is calculated using only Top 1000 members of that industry.
4. Selection: Top 20 stocks.
5. Portfolio: 5% weight per stock, 5% cash interest (30% tax).
"""
...
class PromoterIncShareholderDecRS:
    def __init__(self):
        print("=" * 100)
        print("PROMOTER INC + RETAIL DEC (DOUBLE CONVICTION) + INDUSTRY RS (20 STOCKS)")
        print("=" * 100)
    ...
    def preprocess_data(self):
        ...
        self.shp_with_info = self.shp_df.merge(self.industry_df[['isin', 'industry', 'industry_group']], on='isin',
                                              how='left')
        ...
        # Combined signal over 4 quarters
        # 1. Promoter Increase
        self.shp_with_info['prev_promoter'] = self.shp_with_info.groupby('isin')['promoter_holding_pct'].shift(
            self.LOOKBACK_QUARTERS)
        self.shp_with_info['promoter_buying'] = (self.shp_with_info['promoter_holding_pct'] - self.shp_with_info[
            'prev_promoter']) > 0

        # 2. Shareholder Decrease
        self.shp_with_info['prev_shareholders'] = self.shp_with_info.groupby('isin')['total_shareholders'].shift(
            self.LOOKBACK_QUARTERS)
        self.shp_with_info['shareholders_decreasing'] = (self.shp_with_info['total_shareholders'] - self.shp_with_info[
            'prev_shareholders']) < 0
            
        self.shp_with_info['double_conviction'] = self.shp_with_info['promoter_buying'] & self.shp_with_info['shareholders_decreasing']
    ...
    def calculate_selection(self, date):
        ...
        p_slice = self.price_df[
            (self.price_df['date'] <= date) & (self.price_df['date'] > date - pd.Timedelta(days=14))].sort_values(
            'date').groupby('isin').last().reset_index()
        ...
        # 2. DOUBLE CONVICTION FILTER (STRICTLY TOP 1000)
        cut = date - pd.Timedelta(days=120)
        recent = self.shp_with_info[(self.shp_with_info['quarter_date'] >= cut) & (
                    self.shp_with_info['quarter_date'] <= date) & (
                                        self.shp_with_info['isin'].isin(top_1000_isins))].copy()
        recent = recent.sort_values('quarter_date').groupby('isin').last().reset_index()

        # Only stocks with BOTH promoter buying and shareholder decrease
        candidate_data = recent[recent['double_conviction'] == True]
        ...
        for ind in industries:
            ind_isins = self.industry_df[(self.industry_df['industry'] == ind) & (
                self.industry_df['isin'].isin(top_1000_isins))]['isin'].tolist()
            ind_ret = self.get_ret_for_isins(ind_isins, date, self.RS_LOOKBACK_DAYS)
            ind_rs[ind] = ind_ret - bench_ret

        # 4. RANK STOCKS BY INDUSTRY RS + APPLY INDUSTRY CAP
        ...
        # Limit to max 4 stocks per industry
        selected_isins = candidate_data.sort_values(by=['ind_rs', 'mc'], ascending=False) \
                                       .groupby('industry') \
                                       .head(4) \
                                       .head(self.NUM_STOCKS)['isin'].tolist()
    ...
    def run(self):
        ...
        print(f"\nPROMOTER INC + RETAIL DEC RETURN: {ret:.2f}%")
        p = self.base_path / 'strategies' / 'new_research' / 'outputs'
        p.mkdir(parents=True, exist_ok=True)
        res.to_csv(p / f'promoter_inc_ret_dec_rs_equity_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv', index=False)


if __name__ == "__main__":
    PromoterIncShareholderDecRS().run()
