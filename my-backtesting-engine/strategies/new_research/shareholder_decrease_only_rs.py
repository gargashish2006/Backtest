#!/usr/bin/env python
"""
Shareholder Decrease ONLY Strategy

Logic:
1. Filter: Stocks where total shareholders DECREASED over a 4-quarter lookback.
...
class ShareholderDecreaseOnlyRS:
    def __init__(self):
        print("=" * 100)
        print("SHAREHOLDER DECREASE ONLY + INDUSTRY RS (20 STOCKS)")
        print("=" * 100)
    ...
    def calculate_selection(self, date):
        ...
        # 2. SENTIMENT FILTER (STRICTLY TOP 1000)
        ...
        selected_isins = candidate_data.sort_values(by=['ind_rs', 'mc'], ascending=False).head(self.NUM_STOCKS)[
            'isin'].tolist()
    ...
    def run(self):
        ...
        print(f"\nSHAREHOLDER DECREASE ONLY RETURN: {ret:.2f}%")
        p = self.base_path / 'strategies' / 'new_research' / 'outputs'
        p.mkdir(parents=True, exist_ok=True)
        res.to_csv(p / f'shareholder_decrease_only_rs_equity_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv', index=False)


if __name__ == "__main__":
    ShareholderDecreaseOnlyRS().run()
