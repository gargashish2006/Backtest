...
class CS15MinRSI8WStrategy(CS15Strategy):
    """Subclass of CS15Strategy that requires min(RSI) over last 8 weeks > 40 for entry."""
    
    def calculate_selection(self, date: pd.Timestamp) -> Dict[str, float]:
        """CS15 Full Selection Logic with 1-Week Offset and Min 8-Week RSI Persistence."""
        ...
        # --- [NEW PERSISTENT RSI ENTRY FILTER] ---
        if not self.rsi_cache.empty:
            # Find the 8 most recent weekly RSI dates on or before actual_signal_date
            rsi_dates_8w = [d for d in self.rsi_cache.index if d <= actual_signal_date][-8:]
            if len(rsi_dates_8w) == 8:
                min_rsi_8w = self.rsi_cache.loc[rsi_dates_8w].min()
                passed_rsi = min_rsi_8w[min_rsi_8w > self.rsi_threshold].index.tolist()
                universe = universe[universe['isin'].isin(passed_rsi)]
            else:
                # If not enough history, fallback to current rule or return empty
                # We'll return empty to be strict about "8-week persistence"
                return {}
...
def run_research():
    ...
    # 1. Setup Strategy (Min RSI last 8 weeks > 40)
    strategy = CS15MinRSI8WStrategy(dh)
    ...
    # 3. Run
    print("Running CS15 (Min 8W RSI > 40) Backtest...")
    ...
    print("\n" + "="*40)
    print("CS15 (MIN 8W RSI > 40) PERFORMANCE")
    print("="*40)
