...
def run_simulation(dh, group_pct, ind_abs, scale_factor=10000.0):
    ...
    strategy = StructuralAlphaGroupStrategy(dh, num_stocks=30, max_per_industry=3, 
                                            ...
                                            industry_absolute_threshold=ind_abs)
...
def run_dual_filter_sweep():
    repo_root = Path("/Users/shubhrakasana/Desktop/oleander/olgo_mobile/repos/Backtest/precision-backtest-engine")
    dh = DataHandler(repo_root / "database/price_data_cleaned.parquet")
    dh.load_data()
    
    test_pairs = [
        (0.35, 0.30, "Rel 35% / Abs 30%"),
        (0.45, 0.40, "Rel 45% / Abs 40%"),
        (0.55, 0.50, "Rel 55% / Abs 50%")
    ]
    ...
    for rel, abs_thr, label in test_pairs:
        print(f"Running Dual-Filter: {label}...")
        nav_series = run_simulation(dh, rel, abs_thr)
        ...
        for y in years:
            ...
            threshold_rets[y] = ret * 100
        yearly_results[label] = threshold_rets

    df_yearly = pd.DataFrame(yearly_results)
    print("\nDUAL-FILTER TERMINAL NAV COMPARISON:")
    ...
    # Plotting
    ax = df_yearly.plot(kind='bar', figsize=(12, 6), width=0.8)
    plt.title("Dual-Filter Sensitivity: Relative Group + Absolute Industry")
    plt.ylabel("Annual Return (%)")
    plt.grid(axis='y', alpha=0.3)
    plt.tight_layout()
    
    plot_path = repo_root / "dual_filter_sensitivity.png"
    plt.savefig(plot_path)
    print(f"\nPlot saved to: {plot_path}")

if __name__ == "__main__":
    run_dual_filter_sweep()
