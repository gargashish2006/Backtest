...
def run_variation():
    ...
    print(f"\nRunning Champion Strategy (1.5Yr History Filter) - {start_date} to {end_date}...")
    
    port = Portfolio(10000000)
    # MIN HISTORY = 1.5 YEARS
    strategy = ContrarianBreadthStrategy(dh, min_history_years=1.5)
    ...
    print(f"\n  >>> Variation Result (2Yr Hist): CAGR {stats['CAGR']}, Sharpe {stats['Sharpe Ratio']}, DD {stats['Max Drawdown']}")
    
    # Save NAV for comparison
    output_path = repo_root / "outputs/champion_2yr_nav.csv"
