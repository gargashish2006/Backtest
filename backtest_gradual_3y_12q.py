...
def run_gradual_3y_12q():
    ...
    q_allot = (100.0 / 12.0) * scale_factor # ~8.3333
    ...
    print(f"Starting Mega-Long Gradual Entry (30 stocks, 8.33% x 12) from {start_date.date()}...")
    ...
    for date in sim_dates:
        ...
        if current_q_idx < 12:
            bench_uninvested = (12 - current_q_idx) * q_allot
        ...
        if date in rebalance_dates:
            ...
            if current_q_idx <= 12:
                ...
                # 3-Year Rolling (Hold 12 Quarters)
                old_q_idx = current_q_idx - 12
                old_map = tranche_qtys.get(old_q_idx, {})
                
                if old_map:
                    print(f"Rolling 3-Year Tranche {old_q_idx}...")
    ...
    plt.plot(df_nav['nav'] / scale_factor, label="Mega-Long Strategy (30 stock, 3Y Hold)")
    plt.plot(df_bench['nav'] / scale_factor, label="Staggered Benchmark (12Q Entry then Static)", alpha=0.7, color='gray')
    plt.title(f"Mega-Long Gradual Entry (3Y Hold / 12Q Stagger)\nStrategy: {final_nav:.1f} | Bench: {final_bench:.1f}")
    plt.ylabel("Portfolio Value (Base 100)")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(repo_root / "gradual_3y_12q_results.png")
    
    print(f"\nMEGA-LONG GRADUAL ENTRY COMPLETE")
