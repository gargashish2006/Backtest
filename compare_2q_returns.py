...
def compare_rebalances():
    ...
    var_path = repo_root / "outputs/lookback_2q_nav.csv"
    ...
    merged = pd.merge(champ_df, var_df, on='date', suffixes=('_champ', '_2q'))
    ...
    print("\n" + "="*80)
    print(f"{'Period End':<15} | {'Champ (1Y)':>12} | {'2Q (6M)':>12} | {'Diff':>12}")
    print("-" * 80)
    
    for _, row in res_df.iterrows():
        print(f"{row['Period End']:<15} | {row['Champion (1Y)']:.2%}       | {row['2-Quarter (6M)']:.2%}       | {row['Diff (Champ - Var)']:.2%}")
    ...
    out_path = repo_root / "outputs/rebalance_comparison_2q.csv"
