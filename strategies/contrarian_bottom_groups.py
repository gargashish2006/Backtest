...
class ContrarianBottomGroupsStrategy(ContrarianBreadthStrategy):
    """
    Subclass of ContrarianBreadthStrategy that selects BOTTOM 50% industry groups
    instead of the Top 50% (Champion).
    """
    def __init__(self, data_handler: DataHandler,
                 ...
                 industry_group_top_pct: float = 0.50, # In this mode, we pick bottom %
    ...
    def calculate_selection(self, date: pd.Timestamp) -> Dict[str, float]:
        """
        Overrides calculate_selection to use BOTTOM groups logic.
        """
        # 1. Calculation dates
        ...
        # (i) Industry Group Filter - THE BOTTOM MODE
        ...
        # DIFFERENT: Sort Ascending to get BOTTOM groups
        bottom_groups = group_stats.sort_values('mean', ascending=True).head(num_to_pick)['group'].tolist()
        
        # (ii) Industry Filter
        ind_in_groups = sh_trend[sh_trend['group'].isin(bottom_groups)]
