...
class SLT15IndustryRankStrategy:
    ...
    def get_qualified_industries_with_scores(self, date: pd.Timestamp) -> Dict[str, float]:
        """Returns dict of {industry: industry_score} for industries passing the 35/35 filter."""
        ...
        top_inds = ind_stats.sort_values(ascending=False).head(max(1, int(len(ind_stats) * 0.35)))
        
        res = {}
        for ind, score in top_inds.items():
            res[ind] = float(score)
                
        return res

    def calculate_selection(self, date: pd.Timestamp) -> Dict[str, float]:
        """Calculates selection using Industry Breadth ranking."""
        ind_to_score = self.get_qualified_industries_with_scores(date)
        if not ind_to_score: return {}

        # 1. Rank the qualified industries by their INDUSTRY score (highest = most decreased)
        sorted_industries = sorted(ind_to_score.items(), key=lambda item: item[1], reverse=True)
