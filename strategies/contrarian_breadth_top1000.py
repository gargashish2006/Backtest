...
class ContrarianBreadthTop1000Strategy:
    """
    Champion Strategy: Contrarian Breadth Strategy with Top 1000 Breadth Filter
    - Universe: Top 1000 stocks by Market Cap
    - Signals: Global Shareholding Decrease (1-Year)
    - Industry Group: Top 50% by internal breadth (CALCULATED ONLY ON TOP 1000 UNIVERSE)
    - Industry: Minimum 50% breadth within top groups (CALCULATED ONLY ON TOP 1000 UNIVERSE)
    ...
    def calculate_selection(self, date: pd.Timestamp) -> Dict[str, float]:
        """Full rebalance logic with RESTORED Top 1000 Breadth Filter."""
        ...
        # 3. Shareholder Filter (RESTORED RESTRICTION TO UNIVERSE)
        sh_trend = self.dh.get_shareholder_trend(date, lookback_quarters=self.shareholder_lookback_quarters)
        if sh_trend.empty: return {}
        
        # RESTORATION POINT: Apply shareholder breadth ONLY for top 1000 stocks (universe)
        # This is what restores the 22.54% CAGR by excluding Industries that don't satisfy breadth in Large Caps.
        ...
        if self.rsi_threshold > 0 and not self.rsi_cache.empty:
             ...
             if valid_cache_dates:
                 ...
                 valid_isins = [i for i in universe['isin'] if i in self.rsi_cache.columns]
                 if valid_isins:
        ...
        return {isin: 1.0 / num_final for isin in selected_isins}
