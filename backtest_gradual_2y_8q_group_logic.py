...
def run_gradual_group_logic():
    ...
    # Strategy Config: 30 stocks, 3 max per industry, Top 50% Groups, 8Q SL
    strategy = StructuralAlphaGroupStrategy(dh, num_stocks=30, max_per_industry=3, shareholder_lookback_quarters=8)
    ...
    print(f"Starting Hierarchical Gradual Entry (30 stocks, Group-Filter, 8Q SL) from {start_date.date()}...")
    ...
    plt.plot(df_nav['nav'] / scale_factor, label="Hierarchical Group Strategy (Top 50% Groups, 2Y Hold, 8Q lookback)")
    plt.plot(df_bench['nav'] / scale_factor, label="Staggered Benchmark (8Q Entry)", alpha=0.7, color='gray')
    plt.title(f"Hierarchical Structural Strategy (Group-Filter)\nStrategy: {final_nav:.1f} | Bench: {final_bench:.1f}")
    plt.ylabel("Portfolio Value (Base 100)")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(repo_root / "gradual_2y_8q_group_results.png")
    
    print(f"\nHIERARCHICAL STRUCTURAL SIMULATION COMPLETE")
    print(f"Final Strategy Value: {final_nav:.2f}")
    print(f"Final Benchmark Value: {final_bench:.2f}")

if __name__ == "__main__":
    run_gradual_group_logic()
