...
from strategies.slt15_pure_median_strategy import SLT15PureMedianStrategy
...
def run_pure_median_compare():
    ...
    print("\nRunning SLT15_12Q Original (Breadth Filter + Individual Rank)...")
    slt15_orig = SLT15Strategy(dh, lookback_quarters=12)
    nav_orig = run_strategy_sim(dh, slt15_orig, hold_quarters=8)
    
    print("\nRunning SLT15_12Q Median Filter (Median Filter + Individual Rank)...")
    slt15_med_filt = SLT15MedianFilterStrategy(dh, lookback_quarters=12)
    nav_med_filt = run_strategy_sim(dh, slt15_med_filt, hold_quarters=8)
    
    print("\nRunning SLT15_12Q Pure Median (Median Filter + Pure Median Rank)...")
    slt15_pure = SLT15PureMedianStrategy(dh, lookback_quarters=12)
    nav_pure = run_strategy_sim(dh, slt15_pure, hold_quarters=8)
    ...
    nav_med_filt = nav_med_filt[nav_med_filt.index >= start_date] / nav_med_filt.iloc[0] * 100
    nav_pure = nav_pure[nav_pure.index >= start_date] / nav_pure.iloc[0] * 100
    bench = bench[bench.index >= start_date] / bench.iloc[0] * 100
    
    # Plotting
    plt.figure(figsize=(12, 7))
    plt.plot(nav_orig, label=f"SLT15_12Q Original (NAV: {nav_orig.iloc[-1]:.1f})", color='navy', linewidth=2)
    plt.plot(nav_med_filt, label=f"SLT15_12Q Median Filter (NAV: {nav_med_filt.iloc[-1]:.1f})", color='darkorange', linestyle='--', linewidth=2)
    plt.plot(nav_pure, label=f"SLT15_12Q Pure Median (NAV: {nav_pure.iloc[-1]:.1f})", color='firebrick', linestyle='-.', linewidth=2)
    plt.plot(bench, label="Top 1000", color='gray', linestyle=':', alpha=0.5)
    plt.title("SLT15_12Q Structural Logic Showdown (May 2019 - Feb 2026)")
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    plot_path = repo_root / "outputs/slt15_pure_median_comparison.png"
    plt.savefig(plot_path)
    print(f"\nComparison plot saved to: {plot_path}")
    
    # Metrics
    metrics = {
        "SLT15_12Q (Original Breadth)": calculate_metrics(nav_orig),
        "SLT15_12Q (Median Filter / Indiv Rank)": calculate_metrics(nav_med_filt),
        "SLT15_12Q (Pure Median Filter/Rank)": calculate_metrics(nav_pure),
        "Benchmark": calculate_metrics(bench)
    }
    
    df_metrics = pd.DataFrame(metrics).T
    print("\nStructural Filtering & Ranking Methodology Metrics:")
    print(df_metrics[['Final NAV', 'CAGR', 'Sharpe', 'MaxDD']].to_string())

if __name__ == "__main__":
    run_pure_median_compare()
