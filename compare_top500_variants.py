"""
Frictionless comparison of 5 Top-500 variants:
1. Baseline (Index)
2. SH Decrease (1Q)
3. SH Decrease (4Q)
4. MCPS Increase (1Q)
...
class Top500VariantStrategy:
    ...
    def calculate_selection(self, date):
        # 1. Get Top 500 Universe by Market Cap
        top_500 = self.dh.get_universe(date, size=500)
        if top_500.empty: return {}
        
        if self.mode == 'baseline':
            isins = top_500['isin'].tolist()
        else:
            ...
            merged = pd.merge(top_500, curr_sh, on='isin')
...
def run_sim(mode):
    ...
    strat = Top500VariantStrategy(dh, mode)
...
ax.set_title("Top 500 Selection Variants: SH cleaning vs. MCPS Growth", color='white', fontsize=18)
ax.set_ylabel("Normalized NAV", color='white')
ax.legend(facecolor='#1a1a2e', labelcolor='white')
ax.grid(True, color='#222222')
ax.tick_params(colors='white')

out_path = repo_root / "top500_variants_comparison.png"
plt.tight_layout()
