"""
MCPS15 Variant B + Shareholder Breadth Filter (>= 50% stocks with decreasing shareholders)
Uses corrected Variant B MCPS (signal-date M-Cap now vs T-7 minus 1yr for prev).
Industries must pass the shareholder breadth filter BEFORE being ranked by MCPS %.
"""
import pandas as pd
...
def get_quarter_info(signal_date, lookback_quarters=4):
    ...
    prev_code = ["Mar","Jun","Sep","Dec"][linear_prev % 4]
...
class MCPS15WithBreadthFilter:
    """
    MCPS15 Variant B + Shareholder Breadth Filter:
    1. Filter: only industries where >= sh_decrease_min_pct of stocks have decreasing shareholders
    2. Rank surviving industries by % of stocks with positive MCPS change (Variant B M-Cap)
    3. Select top 3 stocks by M-Cap per industry, up to 15 total
    """

    def __init__(self, data_handler: DataHandler,
                 sh_decrease_min_pct: float = 0.50,
                 num_stocks: int = 15,
        ...
        self.sh_decrease_min_pct = sh_decrease_min_pct
    ...
    def precompute_rsi(self, dates):
        print(f"Pre-computing Weekly RSI Cache for MCPS15+Breadth...")
    ...
    def calculate_selection(self, date: pd.Timestamp):
        ...
        # Step 1: Shareholder breadth filter — industries with >= 50% stocks decreasing
        ind_breadth = merged.groupby('industry')['decreased'].mean().reset_index()
        qualified_industries = ind_breadth[ind_breadth['decreased'] >= self.sh_decrease_min_pct]['industry'].tolist()
        if not qualified_industries: return {}

        # Step 2: MCPS ranking (Variant B) within qualified industries only
        ...
        # Only rank industries that passed breadth filter
        filtered = merged[merged['industry'].isin(qualified_industries)]
        if filtered.empty: return {}

        ind_mcps = filtered.groupby('industry').agg(
            mcps_positive_pct=('mcps_positive', 'mean')
        ).reset_index().sort_values('mcps_positive_pct', ascending=False)

        # Step 3: Universe + liquidity + RSI filters
...
# ── Run variants ──────────────────────────────────────────────────────────────
variants = [
    ("MCPS15-B (no breadth filter)", MCPS15WithBreadthFilter(dh, sh_decrease_min_pct=0.0)),
    ("MCPS15-B + Breadth >= 50%",    MCPS15WithBreadthFilter(dh, sh_decrease_min_pct=0.50)),
]
results, navs = {}, {}
...
ax.set_title('MCPS15-B + Shareholder Breadth Filter (≥50%) vs CS15', fontsize=14,
...
out_path = repo_root / "mcps15_breadth_filter.png"
plt.tight_layout()
