"""
Frictionless comparison of 5 selection variants for Rank 251 to Rank 1000:
1. Baseline (Index)
2. SH Decrease (1Q)
3. SH Decrease (4Q)
4. MCPS Increase (1Q)
...
class ExTop250VariantStrategy:
    ...
    def calculate_selection(self, date):
        # 1. Get Top 1000 Universe and slice for Rank 251-1000
        full_top_1000 = self.dh.get_universe(date, size=1000)
        if full_top_1000.empty: return {}
        
        # Sort by MC to ensure exact ranking
        full_top_1000 = full_top_1000.sort_values('mc', ascending=False)
        ex_250_universe = full_top_1000.iloc[250:] # Rank 251 to 1000
        
        if ex_250_universe.empty: return {}
        
        if self.mode == 'baseline':
            isins = ex_250_universe['isin'].tolist()
        else:
            ...
            merged = pd.merge(ex_250_universe, curr_sh, on='isin')
...
def run_sim(mode):
    ...
    strat = ExTop250VariantStrategy(dh, mode)
...
ax.set_title("Ex-Top 250 (Rank 251-1000) Selection Variants: SH cleaning vs. MCPS Growth", color='white', fontsize=18)
ax.set_ylabel("Normalized NAV", color='white')
ax.legend(facecolor='#1a1a2e', labelcolor='white')
ax.grid(True, color='#222222')
ax.tick_params(colors='white')

out_path = repo_root / "ex_top250_variants_comparison.png"
plt.tight_layout()
