...
def compare_rebalances():
    ...
    var_path = repo_root / "outputs/top100_nav.csv"
    ...
    merged = pd.merge(champ_df, var_df, on='date', suffixes=('_champ', '_top100'))
    ...
    for curr_date in rebalance_schedule[1:]:
        ...
        results.append({
            ...
            "Champion (1000)": ret_champ,
            "Top 100 Univ": ret_var,
            "Diff": diff
        })
    ...
    print("\n" + "="*80)
    print(f"{'Period End':<15} | {'Champ (1000)':>12} | {'Top 100 Univ':>12} | {'Diff':>12}")
    print("-" * 80)
    
    for _, row in res_df.iterrows():
        print(f"{row['Period End']:<15} | {row['Champion (1000)']:.2%}       | {row['Top 100 Univ']:.2%}       | {row['Diff']:.2%}")
    ...
    out_path = repo_root / "outputs/rebalance_comparison_top100.csv"
