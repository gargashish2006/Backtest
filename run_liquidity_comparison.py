...
def run_liquidity_comparison():
    ...
    variations = [
        ("Average Traded Value", 'avg'),
        ("Median Traded Value", 'median'),
        ("Minimum Traded Value", 'min')
    ]
    
    for label, mode in variations:
        print(f"\n--- Running Variation: {label} ---")
        port = Portfolio(10000000)
        # Using Original Champion (0y filter)
        strategy = ContrarianBreadthStrategy(dh, min_history_years=0.0, liquidity_mode=mode)
    ...
    print("\n" + "="*60)
    print(f"LIQUIDITY FILTER VARIATION RESULTS (2017 - 2026)")
    print("="*60)
