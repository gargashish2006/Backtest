"""
Alignment Benchmark Switching Backtest (Frictionless)
Rule (Reversed): 
- If Breadth(Top 200) > Breadth(Mid), Buy Top 200
- If Breadth(Mid) > Breadth(Top 200), Buy Top 1000
"""
import pandas as pd
import numpy as np
...
for i, rd in enumerate(rdates):
    ...
    # Switcher Logic (REVERSED)
    # If B(200) > B(Mid) -> Buy Top 200
    if b_a > b_b:
...
print(f"{'Alignment Switcher':<20} | {get_stats(df_hist['NAV_Switch'])[0]:^15} | {get_stats(df_hist['NAV_Switch'])[1]:^15}")
print("=" * 60)
...
ax.plot(df_hist.index, df_hist['NAV_Switch'] / initial_cash * 100, color='#ff9f1c', linewidth=3, label='Alignment Switcher (Reversed)')
...
ax.set_title("Alignment Benchmark Switcher vs Static Benchmarks", color='white', fontsize=16)
ax.tick_params(colors='white')
ax.grid(True, color='#222222')
ax.legend(facecolor='#1a1a2e', labelcolor='white')

out_path = repo_root / "alignment_benchmark_switcher.png"
plt.tight_layout()
