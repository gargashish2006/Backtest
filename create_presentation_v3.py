...
def create_full_chart_slide(prs, title, subtitle, image_path, bullets=[], page=0):
    """Full width chart slide with minimal bullets over/under/side."""
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_slide_background(slide)
    add_master_layout_elements(slide)
    add_footer(slide, page)
    ...
    # Bullets on right if any
    if bullets:
        b_box = slide.shapes.add_textbox(Inches(9.5), Inches(2), Inches(3.5), Inches(4))
        tf = b_box.text_frame
        tf.word_wrap = True
        for item in bullets:
            p = tf.add_paragraph()
...
# Slide 6: Long-Term Performance (9-Year Backtest)
create_full_chart_slide(
    prs,
    "Long-Term Validation",
    "Full Cycle Performance (2017-2026)",
...
# Slide 7: Yearly Returns breakdown
create_full_chart_slide(
    prs,
    ...
    bullets=[
        "Consistent positive Alpha across most years.",
        "Significant outperformance in Volatile/Bear markets.",
        "Demonstrates robustness of the quantitative logic."
    ],
    page=7
)

# Slide 8: Dynamic Edge (Conclusion)
...
# Save
output_path = '/Users/shubhrakasana/Desktop/oleander/olgo_mobile/repos/Backtest/precision-backtest-engine/Champion_Strategy_Presentation_v3.pptx'
prs.save(output_path)
print(f"Presentation v3 saved to: {output_path}")
