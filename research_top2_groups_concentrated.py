...
class Top2GroupsConcentratedStrategy(ContrarianBreadthStrategy):
    """
    Variation:
    1. Select Top 2 Industry Groups.
    2. NO max_per_industry limit (can pick all 15 stocks from one industry).
    3. NO 10% weight cap per stock.
    """
    def calculate_selection(self, date: pd.Timestamp) -> Dict[str, float]:
        ...
        for ind in ind_ranked['industry']:
            ...
            # VARIATION: NO LIMIT per industry (pass self.num_stocks as limit effectively)
            top_for_ind = ind_universe.head(self.num_stocks)['isin'].tolist()
...
def run_top2_concentrated():
    ...
    print("Running Top 2 Groups CONCENTRATED Variation (No Limits)...")
    warnings.filterwarnings('ignore')

    strat = Top2GroupsConcentratedStrategy(dh,
                               num_stocks=15, 
                               max_per_industry=15, # Effectively no limit
                               universe_size=1000, 
                               liquidity_threshold_pct=0.00005,
                               industry_group_top_pct=0.50, # Ignored
    ...
    output_path = repo_root / "outputs/top2_conc_nav.csv"
    nav_df.to_csv(output_path, index=False)
    
    stats = calculate_metrics(nav_df)
    
    print("\n" + "="*60)
    print("TOP 2 CONCENTRATED VARIATION PERFORMANCE")
    print("="*60)
    ...
    print("="*60)
    print(f"{'Metric':<20} | {'Top 2 Conc':>15} | {'Champion':>15}")
    print("-" * 55)
    ...
    if baseline_path.exists():
        ...
        plt.plot(champ_nav, label='Champion (Top 50%)', alpha=0.7)
        plt.plot(var_nav, label='Variation (Top 2 Conc)', linewidth=2)
        plt.title('Strategy Comparison: Top 2 Groups Concentrated vs Champion')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.savefig(repo_root / "outputs/top2_conc_comparison.png")
        print(f"Chart saved to: {repo_root / 'outputs/top2_conc_comparison.png'}")

if __name__ == "__main__":
    run_top2_concentrated()
