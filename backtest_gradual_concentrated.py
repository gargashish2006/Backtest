...
def run_gradual_concentrated():
    ...
    # 1. Setup
    initial_total_capital = 100.0 # User base unit
    ...
    # Strategy Config: Top 10 Industries, 1 stock each
    strategy = StructuralAlphaStrategy(dh, num_stocks=10, max_per_industry=1)
    ...
    print(f"Starting Concentrated Gradual Entry (10 stocks, 25% x 4) from {start_date.date()}...")
    ...
    plt.plot(df_nav['nav'] / scale_factor, label="Concentrated Strategy (10 stocks, Top 10 Ind)")
    plt.plot(df_bench['nav'] / scale_factor, label="Staggered Benchmark (Static)", alpha=0.7)
    plt.title(f"Concentrated Gradual Entry: Top 10 Industries\nStrategy: {final_nav:.1f} | Bench: {final_bench:.1f}")
    plt.ylabel("Portfolio Value (Base 100)")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(repo_root / "gradual_concentrated_results.png")
    
    print(f"\nCONCENTRATED GRADUAL ENTRY COMPLETE")
    print(f"Final Strategy Value: {final_nav:.2f}")
    print(f"Final Benchmark Value: {final_bench:.2f}")

if __name__ == "__main__":
    run_gradual_concentrated()
